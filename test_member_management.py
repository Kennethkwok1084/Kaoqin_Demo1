...
def test_model_structure():
    ...
    try:
        ...
        missing_fields = [
            field for field in expected_fields if field not in member_fields
        ]
        if not missing_fields:
            print_test_result("Member模型字段检查", True, f"所有必要字段存在")
        else:
            print_test_result(
                "Member模型字段检查", False, f"缺少字段: {missing_fields}"
            )
        ...
        print_test_result(
            "Member模型方法检查",
            len(existing_methods) == len(methods),
...
def test_business_logic():
    ...
    try:
        ...
        print_test_result(
            "数据导入权限检查", member.can_import_data, "数据导入权限正确"
        )
...
def test_import_functionality():
    ...
    try:
        ...
        print_test_result(
            "用户名生成逻辑",
            len(generated_usernames) == len(import_items),
