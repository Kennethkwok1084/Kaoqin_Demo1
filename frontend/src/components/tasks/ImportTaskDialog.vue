  ...
  >
        ...
        <div class="step-header">
          <el-card>
            ...
            <div class="step-description">
              <p>
                请先导入A表（报修单），包含工单编号、报修人、联系方式、故障描述等基础信息。
              </p>
              <el-alert type="info" :closable="false">
                <strong>重要说明：</strong>A表必须先导入，作为后续
                B表匹配的基础数据。系统将以"报单人+联系电话"为关键字对A-B表进行匹配。
              </el-alert>
            </div>
          </el-card>
        </div>
        ...
        <div class="template-section">
          <el-card>
            ...
            <div class="format-info">
              <h4>A表（报修单）导入格式说明：</h4>
              <el-table :data="aTableColumns" stripe max-height="300">
              ...
              >
                <ul class="notice-list">
                  <li>
                    <strong>必填字段：</strong
                    >工单编号、位置、对象、描述、报单人、报单人电话、报单时间、工单状态
                  </li>
                  <li>
                    <strong>报单企业：</strong
                    >可以为空，系统会根据其他信息进行推断
                  ...
                  <li>
                    <strong>联系方式映射：</strong>联系方式对应报单人电话字段
                  </li>
                  <li>
                    <strong>时间格式：</strong>报单时间对应报修时间，格式为
                    YYYY-MM-DD HH:mm:ss
                  </li>
                  <li><strong>状态字段：</strong>工单状态对应工单状态列</li>
                  <li>
                    <strong>任务类型：</strong
                    >系统根据"类型"字段自动识别线上/线下任务
                  </li>
                  <li>
                    <strong>匹配关键字：</strong
                    >"报单人+报单人电话"将作为A-B表匹配的唯一标识
                  </li>
                </ul>
              </el-alert>
            </div>
            ...
            <template #header>
              <div class="step-title">
                ...
                <el-tag type="success" size="small" style="margin-left: 8px"
                  >A表已导入{{ aTableData.length }}条记录</el-tag
                >
              </div>
            </template>
            <div class="step-description">
              <p>
                可选择导入B表（维护记录），包含处理时间、处理人、工时、评价等维护处理信息。
              </p>
              ...
              >
                <strong>匹配原理：</strong
                >系统将以B表中的"联系人+联系方式"与A表中的"报单人+报单人电话"进行全匹配，生成完整的维修任务记录。
              </el-alert>
              ...
              >
                <strong>跳过说明：</strong
                >跳过B表导入后，所有A表记录将自动设置为线上任务类型，按40分钟/单计算工时，无处理人员和维护详情信息。
              </el-alert>
            </div>
          </el-card>
        </div>
            ...
            <el-card>
              <div class="file-details">
                ...
                <div class="file-meta">
                  ...
                  <div class="file-size">
                    {{ formatFileSize(selectedFileB.size) }}
                  </div>
                </div>
        ...
        <div v-if="bTableImportOption === 'import'" class="template-section">
          <el-card>
            ...
            <div class="format-info">
              ...
              <el-table :data="bTableColumns" stripe max-height="300">
                <el-table-column prop="field" label="字段名" width="120" />
                <el-table-column prop="description" label="说明" />
                <el-table-column prop="required" label="必填" width="80">
                  <template #default="{ row }">
              ...
              >
                <ul class="notice-list">
                  <li>
                    <strong>必填字段：</strong
                    >工单编号、联系人、联系方式（用于与A表匹配）
                  </li>
                  <li>
                    <strong>A/B表匹配：</strong
                    >系统以"联系人+联系方式"为关键字进行匹配
                  </li>
                  <li>
                    <strong>工时计算：</strong
                    >线上任务40分钟/单，线下任务100分钟/单，支持爆单标记+15分钟
                  </li>
                  <li>
                    <strong>时间格式：</strong>YYYY-MM-DD HH:mm:ss 或 YYYY/MM/DD
                    HH:mm:ss
                  </li>
                  <li>
                    <strong>处理记录：</strong
                    >处理人、处理时间、完成时间、工时等维护记录
                  </li>
                  <li>
                    <strong>评价处理：</strong
                    >客户评价支持"满意"、"不满意"等文字，满意度支持1-5数字评分
                  </li>
                </ul>
              </el-alert>
            </div>
              ...
              <el-table-column label="处理信息" width="120">
                <template #default="{ row }">
                  <div class="process-info">
                    ...
                    <div
                      v-else-if="row.completionStatus"
                      class="completion-status"
                    >
                      {{ row.completionStatus }}
                    </div>
                    <div v-if="row.actualHours" class="work-hours">
                      {{ row.actualHours }}小时
                    </div>
              ...
              <div class="result-stats">
                <el-card>
                  <el-descriptions :column="2" border>
                    ...
                    <el-descriptions-item
                      label="处理人匹配"
                      v-if="importResult.matched_assignees"
                    >
                      <el-tag type="info"
                        >{{ importResult.matched_assignees }}个</el-tag
                      >
                    </el-descriptions-item>
                  </el-descriptions>
                </el-card>
                ...
                <div
                  v-if="importResult.assignee_matches?.length"
                  class="assignee-matches"
                  style="margin-top: 16px"
                >
                  <h4>处理人匹配详情：</h4>
                  <el-card>
                    <ul class="match-list">
                      <li
                        v-for="(match, index) in importResult.assignee_matches"
                        :key="index"
                        class="match-item"
                      >
    ...
    <template #footer>
      <div class="dialog-footer">
        ...
        <el-button v-if="currentStep === 2" @click="handlePrevious">
          上一步
        </el-button>
...
const bTableColumns = [
  {
    field: '工单编号',
    description: '对应A表的工单编号',
    required: true,
    example: 'WO202401001'
  },
  {
    field: '联系人',
    description: '对应A表的报单人',
    required: true,
    example: '张老师'
  },
  {
    field: '联系方式',
    description: '对应A表的报单人电话',
    required: true,
    example: '13800138000'
  },
  {
  ...
  },
  {
    ...
    description: '任务完成时间',
    required: false,
    example: '2024-01-15 16:30:00'
  },
  {
    field: '故障描述',
    description: '技术人员故障描述',
    required: false,
    example: '硬盘损坏导致系统无法启动'
  },
  {
    field: '解决方案',
    description: '具体解决方法',
...
const handleParseBOrSkip = async () => {
  try {
    ...
    if (bTableImportOption.value === 'skip') {
      ...
      currentStep.value = 2
      ElMessage.success(
        `已跳过B表导入，A表${aTableData.value.length}条记录将全部按线上单处理`
      )
    } else {
      // 正常B表导入流程
      ...
      currentStep.value = 2
      ElMessage.success(
        `B表解析成功，匹配完成。A表${aTableData.value.length}条，B表${bTableData.value.length}条，成功匹配${matchedCount.value}条`
      )
    }
  } catch (error) {
...
const handleImport = async () => {
  try {
    ...
    console.log(
      `A-B表匹配导入完成：成功${importResult.success}条，失败${importResult.failed}条`
    )
    currentStep.value = 3
  } catch (error) {
