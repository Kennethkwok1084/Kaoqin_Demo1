...
async function waitForBackendReady(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      // 检查健康端点
      const healthResponse = await apiContext.get(
      ...
      if (healthResponse.ok()) {
        ...
        // 检查数据库连接
        const dbResponse = await apiContext.get(
...
async function initializeTestData(
  ...
  try {
    // 创建测试用户
    const userResponse = await apiContext.post(
    ...
    // 创建管理员用户
    const adminResponse = await apiContext.post(
      'http://localhost:8000/api/v1/auth/register',
      {
        data: {
          username: 'admin',
    ...
    console.warn(
      '⚠️ Test data initialization had issues, but continuing...',
      error
    )
  }
}
...
async function setupAuthentication(page: any, baseURL: string): Promise<void> {
  try {
    ...
    throw new Error(
      'Failed to setup authentication - this will cause all E2E tests to fail'
    )
  }
}
