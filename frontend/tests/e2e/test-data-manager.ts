...
export class TestDataManager {
  ...
  async getAuthToken(username: string, password: string): Promise<string> {
    const response = await this.apiContext.post('http://localhost:8000/api/v1/auth/login', {
      data: {
        username,
        password
      }
  ...
  async createTestUser(user: TestUser): Promise<void> {
    try {
      const response = await this.apiContext.post('http://localhost:8000/api/v1/auth/register', {
        data: user
      })
  ...
  // 创建测试任务
  async createTestTask(task: TestTask, authToken: string): Promise<string | null> {
    try {
      const response = await this.apiContext.post('http://localhost:8000/api/v1/tasks/', {
        headers: {
  ...
  private async deleteTestTasks(authToken: string): Promise<void> {
    try {
      const response = await this.apiContext.get('http://localhost:8000/api/v1/tasks/', {
        headers: {
          'Authorization': `Bearer ${authToken}`
        }
      })
      ...
      if (response.ok()) {
        ...
        for (const task of tasks) {
          if (task.title.includes('测试')) {
            await this.apiContext.delete(`http://localhost:8000/api/v1/tasks/${task.id}`, {
              headers: {
                'Authorization': `Bearer ${authToken}`
              }
            })
  ...
  private async deleteTestUsers(authToken: string): Promise<void> {
    try {
      ...
      for (const username of testUsernames) {
        try {
          await this.apiContext.delete(`http://localhost:8000/api/v1/users/${username}`, {
            headers: {
              'Authorization': `Bearer ${authToken}`
            }
          })
