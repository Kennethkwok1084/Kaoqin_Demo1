...
test.describe('登录→创建任务→查看工时端到端流程', () => {
  ...
  test('完整业务流程：登录→创建任务→查看工时', async () => {
    ...
    await test.step('创建新任务', async () => {
      ...
      await page.fill('input[placeholder*="报修人姓名"]', testTask.reporter_name)
      await page.fill('input[placeholder*="联系电话"]', testTask.reporter_phone)
      
      // 选择任务类型
      await page.click('.el-select:has-text("任务类型")')
      await page.click(`text=${testTask.task_type === 'repair' ? '维修任务' : '监控任务'}`)
      
      // 选择优先级
      await page.click('.el-select:has-text("优先级")')
      await page.click(`text=${testTask.priority === 'high' ? '高' : testTask.priority === 'medium' ? '中' : '低'}`)
    ...
    await test.step('验证任务出现在列表中', async () => {
      ...
      await expect(page.locator('.task-table')).toContainText(testTask.reporter_name)
    ...
    await test.step('查看任务详情', async () => {
      // 找到刚创建的任务行
      const taskRow = page.locator('.task-table tbody tr').filter({
      ...
      await expect(page.locator('.task-detail-dialog')).toContainText(testTask.title)
      await expect(page.locator('.task-detail-dialog')).toContainText(testTask.description)
      await expect(page.locator('.task-detail-dialog')).toContainText(testTask.location)
    ...
    await test.step('查看新任务的工时详情', async () => {
      ...
      // 验证任务出现在工时列表中
      const workHourRow = page.locator('.work-hours-table tbody tr').filter({
        hasText: testTask.title
      }).first()
      ...
      await expect(page.locator('.work-hour-detail-dialog')).toContainText('工时详情')
      await expect(page.locator('.work-hour-detail-dialog')).toContainText(testTask.title)
      
      // 验证工时计算规则显示
      await expect(page.locator('.work-hour-detail-dialog')).toContainText('基础工时')
    ...
    await test.step('模拟任务状态变更', async () => {
      ...
      // 等待保存成功消息
      await expect(page.locator('.el-message--success')).toContainText('任务更新成功')
    })
    ...
    await test.step('验证状态变更后的工时更新', async () => {
      ...
      await expect(page.locator('.work-hour-detail-dialog')).toBeVisible()
      await expect(page.locator('.work-hour-detail-dialog')).toContainText('进行中')
      
      // 验证工时记录包含状态变更历史
      await expect(page.locator('.work-hour-detail-dialog')).toContainText('状态变更记录')
    })
    ...
    await test.step('清理测试数据', async () => {
      ...
      // 验证任务不再存在于列表中
      await page.waitForTimeout(1000)
      const taskCount = await page.locator('.task-table tbody tr').filter({
        hasText: testTask.title
      }).count()
  ...
  test('无障碍访问：键盘导航', async () => {
    ...
    // 验证表单提交
    await expect(page.locator('button[type="submit"]')).toHaveAttribute('loading', 'true')
  })
})