...
def test_model_imports():
    ...
    try:
        ...
        print_test_result(
            "UserRole枚举值", set(roles) == set(expected_roles), f"角色: {roles}"
        )
...
def test_schema_validation():
    ...
    try:
        ...
        print_test_result(
            "正确数据验证", True, f"用户名: {member.username}, 姓名: {member.name}"
        )
...
def test_member_model_methods():
    ...
    try:
        ...
        print_test_result(
            "can_manage_group方法", admin_member.can_manage_group, "组管理权限检查正确"
        )
        print_test_result(
            "can_import_data方法", admin_member.can_import_data, "数据导入权限检查正确"
        )
        ...
        print_test_result(
            "普通成员权限",
            not member.is_admin and not member.can_import_data,
            "普通成员权限限制正确",
        )
        ...
        print_test_result(
            "显示名称生成", display_name == expected, f"生成: {display_name}"
        )

        # 测试状态显示
        print_test_result(
            "状态显示",
            member.status_display == "在职",
            f"状态: {member.status_display}",
        )
...
def test_import_schema():
    ...
    try:
        from app.schemas.member import MemberImportItem, MemberImportRequest
        ...
        import_items = [
            ...
            MemberImportItem(name="李四", student_id="2023002", class_name="计算机2班"),
        ]

        import_request = MemberImportRequest(members=import_items, skip_duplicates=True)

        print_test_result(
            "导入Schema创建", True, f"成功创建包含{len(import_items)}个成员的导入请求"
        )
        print_test_result(
            "跳过重复设置", import_request.skip_duplicates, "跳过重复功能已启用"
        )
