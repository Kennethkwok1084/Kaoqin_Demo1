...
def fix_file_completely(filepath: str):
    ...
    try:
        ...
        for line in lines:
            ...
            if re.match(r'    async def test_\w+\(self, async_client: AsyncClient, auth_headers\):', line):
                line = line.replace('auth_headers)', 'auth_headers, token):')
            elif re.match(r'    async def test_\w+\(self, async_client: AsyncClient, auth_headers, token\):', line):
                pass  # 已经有token了
            
            # 修复headers = await auth_headers()
            if 'headers = await auth_headers()' in line:
                line = line.replace('headers = await auth_headers()', 'headers = auth_headers(token)')
