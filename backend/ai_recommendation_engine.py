...
class HybridRecommendationEngine:
    ...
    def _merge_recommendations(
        self, rule_recs: List[Recommendation], ai_recs: List[Recommendation]
    ) -> List[Recommendation]:
        """智能合并两种建议"""
        merged = []
        ...
        # 优先处理高优先级的AI建议
        for rec in sorted(
            ai_recs, key=lambda x: self._priority_score(x.priority), reverse=True
        ):
...
class EnhancedRecommendationSystem:
    ...
    def _get_recent_changes(self) -> List[str]:
        """获取最近的代码变更"""
        # 这里可以集成git信息
        return ["修改了用户认证逻辑", "更新了数据库迁移脚本", "优化了API响应格式"]
...
async def main() -> None:
    ...
    test_results = {
        ...
        "performance_metrics": {"average_response_time": 650, "slowest_endpoint": 1200},
        "failures": [
            {
