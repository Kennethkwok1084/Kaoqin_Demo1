...
class TestStatisticsAPIRealCoverage:
    ...
    async def test_get_overview_statistics(self, async_client: AsyncClient, async_session: AsyncSession):
        """测试获取概览统计 - 覆盖系统总体统计逻辑"""
        # 创建管理员用户
        admin = Member(
        ...
        for i, member in enumerate(members):
            for j in range(3):  # 每个成员3个任务
                task = RepairTask(
                    ...
                    status=[TaskStatus.PENDING, TaskStatus.IN_PROGRESS, TaskStatus.COMPLETED][j],
        ...
        for i, member in enumerate(members):
            for days_ago in range(7):  # 每个成员7天的考勤记录
                attendance = AttendanceRecord(
                    ...
                    status=AttendanceStatus.PRESENT if days_ago < 5 else AttendanceStatus.ABSENT,
                    check_in_time=now - timedelta(days=days_ago, hours=16) if days_ago < 5 else None,
                    check_out_time=now - timedelta(days=days_ago, hours=8) if days_ago < 5 else None,
        ...
        # 验证统计查询逻辑执行
        assert response.status_code == 200, f"Overview statistics failed: {response.text}"
        data = response.json()
    ...
    async def test_get_member_performance_statistics(self, async_client: AsyncClient, async_session: AsyncSession):
        ...
        response = await async_client.get(
            f"/api/v1/statistics/members/{test_member.id}/performance",
            headers=headers
        )
    ...
    async def test_get_time_series_statistics(self, async_client: AsyncClient, async_session: AsyncSession):
        """测试获取时间序列统计 - 覆盖趋势分析逻辑"""
        # 创建管理员
        admin = Member(
        ...
        response = await async_client.get("/api/v1/statistics/time-series", 
        ...
        # 验证工作日vs周末的统计差异
        weekday_tasks = [d["task_count"] for d in series_data 
                        if datetime.fromisoformat(d["date"]).weekday() < 5]
        weekend_tasks = [d["task_count"] for d in series_data 
                        if datetime.fromisoformat(d["date"]).weekday() >= 5]
        ...
        weekly_response = await async_client.get("/api/v1/statistics/time-series", 
                                               params=weekly_params, headers=headers)
        
        assert weekly_response.status_code == 200
        weekly_data = weekly_response.json()["data"]
        assert len(weekly_data["series_data"]) == 4  # 4周的数据

    async def test_get_workload_distribution_statistics(self, async_client: AsyncClient, async_session: AsyncSession):
        ...
        for member_idx, task_count in enumerate(workload_distribution):
            for task_idx in range(task_count):
                ...
                task = RepairTask(
                    ...
                    status=TaskStatus.COMPLETED if task_idx < task_count - 2 else TaskStatus.IN_PROGRESS,
                    ...
                    completed_at=now - timedelta(hours=task_idx) if task_idx < task_count - 2 else None,
                    expected_completion_time=now + timedelta(hours=24),
        ...
        # 获取工作负载分布统计
        response = await async_client.get("/api/v1/statistics/workload-distribution", 
                                        headers=headers)
    ...
    async def test_export_statistical_report(self, async_client: AsyncClient, async_session: AsyncSession):
        ...
        for i in range(20):
            ...
            if i < 14:  # 14天的考勤记录
                attendance = AttendanceRecord(
                    ...
                    status=AttendanceStatus.PRESENT if i % 7 != 6 else AttendanceStatus.ABSENT,
                    check_in_time=now - timedelta(days=i, hours=16) if i % 7 != 6 else None,
                    check_out_time=now - timedelta(days=i, hours=8) if i % 7 != 6 else None,
        ...
        response = await async_client.get("/api/v1/statistics/export", 
        ...
        pdf_response = await async_client.get("/api/v1/statistics/export", 
                                            params=pdf_params, headers=headers)

        # 验证PDF导出
        assert pdf_response.status_code == 200
        
        pdf_content_type = pdf_response.headers.get("content-type", "")
        assert "application/pdf" in pdf_content_type or \
               "application/octet-stream" in pdf_content_type
        ...
        json_response = await async_client.get("/api/v1/statistics/export", 
                                             params=json_params, headers=headers)
    ...
    async def test_get_efficiency_trend_analysis(self, async_client: AsyncClient, async_session: AsyncSession):
        ...
        for week in range(8):  # 8周的数据
            ...
            for day in range(5):  # 每周5个工作日
                for task_num in range(3):  # 每天3个任务
                    ...
                    daily_efficiency = max(50, min(100, daily_efficiency))  # 限制在50-100范围
                    
                    task = RepairTask(
                        title=f"效率分析任务W{week}D{day}T{task_num}",
                        description=f"第{week}周第{day}天任务{task_num}",
                        category=TaskCategory.REPAIR,
                        task_type=TaskType.ONLINE if task_num % 2 == 0 else TaskType.OFFLINE,
                        status=TaskStatus.COMPLETED,
                        assignee_id=admin.id,
        ...
        response = await async_client.get("/api/v1/statistics/efficiency-analysis", 
                                        params=params, headers=headers)
