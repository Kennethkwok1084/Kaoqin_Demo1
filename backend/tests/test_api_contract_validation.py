...
class TestMemberApiContract:
    ...
    async def test_member_response_schema(self, async_session):
        ...
        frontend_expected_fields = [
            'id', 'username', 'name', 'student_id', 'email', 'phone',
            'class_name', 'role', 'is_active', 'is_verified', 'created_at'
        ]
...
class TestTaskApiContract:
    ...
    async def test_task_response_schema(self, async_session):
        ...
        # 验证前端期望的字段
        frontend_expected_fields = [
            'id', 'task_id', 'title', 'description', 'task_type', 'status',
            'category', 'priority', 'member_id', 'report_time', 'created_at'
        ]
...
class TestApiResponseWrapperConsistency:
    ...
    async def test_login_response_wrapper_consistency(self):
        ...
        from app.utils.response import create_response
        wrapped_response = create_response(
            data=login_data,
            message="登录成功"
        )
...
class TestFieldMappingValidation:
    ...
    async def test_auth_field_mapping_consistency(self):
        ...
        backend_fields = [
            'student_id', 'class_name', 'is_active', 'is_verified',
            'profile_completed', 'last_login', 'login_count', 
            'created_at', 'updated_at', 'access_token', 'refresh_token'
        ]
        
        # 这些字段应该在前端转换为camelCase
        expected_frontend_fields = [
            'studentId', 'className', 'isActive', 'isVerified',
            'profileCompleted', 'lastLogin', 'loginCount',
            'createdAt', 'updatedAt', 'accessToken', 'refreshToken'
        ]
        ...
        for field in ['student_id', 'class_name', 'is_active', 'is_verified', 'created_at']:
    ...
    async def test_task_field_mapping_consistency(self):
        ...
        # 关键字段应该存在
        critical_fields = [
            'task_number', 'assigned_to', 'estimated_minutes', 'actual_minutes',
