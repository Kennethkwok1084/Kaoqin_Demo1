...
from app.models.task import RepairTask, MonitoringTask, AssistanceTask, TaskStatus, TaskType
...
class TestWorkHoursRecordsAPI(TestAttendanceAPIBasic):
    ...
    @pytest.mark.asyncio
    async def test_get_work_hours_records_basic(self, client, regular_user, sample_repair_task):
        """测试获取工时记录基础功能"""
...
class TestMonthlySummaryAPI(TestAttendanceAPIBasic):
    ...
    async def test_get_monthly_summary_basic(self, client, regular_user):
        ...
        with patch('app.api.deps.get_db') as mock_get_db, \
            ...
            mock_db.execute.side_effect = [
                ...
                MagicMock(fetchone=MagicMock(return_value=mock_repair_stats)),  # 报修统计
                MagicMock(fetchone=MagicMock(return_value=mock_monitoring_stats)),  # 监控统计
                MagicMock(fetchone=MagicMock(return_value=mock_assistance_stats))   # 协助统计
            ]
    ...
    @pytest.mark.asyncio
    async def test_get_monthly_summary_admin_view_others(self, client, admin_user, regular_user):
        """测试管理员查看其他人的月度汇总"""
        with patch('app.api.deps.get_db') as mock_get_db, \
             patch('app.api.deps.get_current_user') as mock_get_user:
...
class TestWorkHoursStatsAPI(TestAttendanceAPIBasic):
    ...
    @pytest.mark.asyncio
    async def test_get_work_hours_stats_admin_view_others(self, client, admin_user, regular_user):
        """测试管理员查看其他人统计"""
        with patch('app.api.deps.get_db') as mock_get_db, \
             patch('app.api.deps.get_current_user') as mock_get_user:
