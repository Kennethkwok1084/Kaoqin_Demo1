...
class TestStatisticsOverviewAPI(TestStatisticsAPIBasic):
    ...
    @pytest.mark.asyncio
    async def test_get_statistics_overview_permission_denied(self, client, regular_user):
        """测试普通用户访问概览API权限拒绝"""
    ...
    async def test_get_efficiency_analysis_basic(self, client, admin_user):
        ...
        with patch('app.api.deps.get_db') as mock_get_db, \
            ...
            mock_efficiency_data = [
                (1, "张三", 65.5, 4.2, 3.5, 10),  # (member_id, name, avg_work_minutes, avg_rating, avg_completion_hours, total_tasks)
...
class TestMonthlyReportAPI(TestStatisticsAPIBasic):
    ...
    async def test_get_monthly_report_basic(self, client, admin_user):
        ...
        with patch('app.api.deps.get_db') as mock_get_db, \
            ...
            # Mock成员排名数据
            mock_member_data = [
                (1, "张三", 800, 15, 4.5),  # (id, name, work_minutes, task_count, avg_rating)
                (2, "李四", 720, 12, 4.2)
            ]
...
class TestChartDataAPI(TestStatisticsAPIBasic):
    ...
    async def test_get_chart_data_different_types(self, client, group_leader_user):
        """测试不同类型的图表数据"""
        chart_types = ["daily_tasks", "member_performance", "category_distribution", "work_hours_trend"]
...
class TestRankingsAPI(TestStatisticsAPIBasic):
    ...
    async def test_get_rankings_basic(self, client, admin_user):
        ...
        with patch('app.api.deps.get_db') as mock_get_db, \
            ...
            # Mock排名数据
            mock_ranking_data = [
                (1, "张三", 1200, 20, 4.8, 1),  # (id, name, work_minutes, task_count, avg_rating, rank)
                (2, "李四", 1000, 18, 4.5, 2),
                (3, "王五", 800, 15, 4.2, 3)
            ]
    ...
    async def test_get_attendance_statistics_basic(self, client, admin_user):
        ...
        with patch('app.api.deps.get_db') as mock_get_db, \
            ...
            mock_attendance_service.get_monthly_statistics.return_value = {
                ...
                "member_statistics": [
                    {"member_id": 1, "name": "张三", "work_days": 20, "work_hours": 150},
                    {"member_id": 2, "name": "李四", "work_days": 19, "work_hours": 142.5}
                ]
            }
