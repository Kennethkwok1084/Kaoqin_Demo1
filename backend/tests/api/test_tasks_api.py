...
class TestAllTasksAPI(TestTasksAPIBasic):
    """测试所有任务API"""

    @pytest.mark.asyncio
    async def test_get_all_tasks_basic(self, client):
        """测试获取所有任务基础功能"""
        with patch('app.api.deps.get_db') as mock_get_db, \
             patch('app.api.deps.get_current_user') as mock_get_user:
            ...
            data = response.json()
            assert data["success"] is True
            assert "repair_tasks" in data["data"]
            assert "monitoring_tasks" in data["data"] 
            assert "assistance_tasks" in data["data"]

    @pytest.mark.asyncio
    async def test_get_all_tasks_with_date_filter(self, client):
        """测试带日期过滤的所有任务查询"""
        with patch('app.api.deps.get_db') as mock_get_db, \
             patch('app.api.deps.get_current_user') as mock_get_user:
...
class TestWorkTimeDetailAPI(TestTasksAPIBasic):
    ...
    @pytest.mark.asyncio
    async def test_get_work_time_detail_with_calculation(self, client, sample_repair_task):
        """测试带工时计算的详情API"""
        with patch('app.api.deps.get_db') as mock_get_db, \
             patch('app.api.deps.get_current_user') as mock_get_user, \
             patch('app.services.work_hours_service.WorkHoursCalculationService') as mock_service:
...
class TestErrorHandling(TestTasksAPIBasic):
    ...
    async def test_service_layer_exception(self, client):
        ...
        with patch('app.api.deps.get_db') as mock_get_db, \
            ...
            # Mock服务层抛出异常
            mock_task_service = AsyncMock()
            mock_task_service.create_repair_task.side_effect = Exception("Service error")
            mock_service.return_value = mock_task_service
