...
class TestAuthLogin:
    ...
    def test_invalid_credentials(
        ...
        response = client_with_db.post("/api/v1/auth/login", json=sample_invalid_login_data)
    ...
    def test_missing_fields(self, client_with_db: TestClient):
        ...
        # Missing password
        response = client_with_db.post("/api/v1/auth/login", json={"student_id": "2021001001"})
        assert response.status_code == 422
        
        # Missing student_id
        response = client_with_db.post("/api/v1/auth/login", json={"password": "password"})
        assert response.status_code == 422
...
class TestTokenRefresh:
    ...
    def test_access_token_as_refresh_token(self, client_with_db: TestClient, test_user: Member):
...
class TestUserProfile:
    ...
    def test_get_profile_with_invalid_token(self, client_with_db: TestClient, auth_headers):
...
class TestPasswordChange:
    ...
    def test_password_change_with_weak_password(
        ...
        assert response.status_code == 422
        assert "Password does not meet strength requirements" in response.json()["detail"]["message"]
...
class TestTokenVerification:
    ...
    def test_verify_expired_token(self, client_with_db: TestClient, test_user: Member, auth_headers):
