...
class TestWorkHourCalculationAlgorithm:
    ...
    async def test_online_task_base_minutes(self, async_session):
        ...
        assert base_minutes == 40, f"Expected 40 minutes for online task, got {base_minutes}"
    ...
    async def test_offline_task_base_minutes(self, async_session):
        ...
        # 测试基础工时计算
        base_minutes = task.get_base_work_minutes()
        assert base_minutes == 100, f"Expected 100 minutes for offline task, got {base_minutes}"
    ...
    async def test_complex_work_hour_calculation(self, async_session):
        ...
        assert total_minutes == expected, f"Expected {expected} minutes, got {total_minutes}"
    ...
    async def test_edge_case_zero_work_minutes(self, async_session):
        ...
        # 添加多个惩罚标签，总惩罚超过基础工时
        penalty_tags = [
            TaskTag(name="延迟响应", work_minutes_modifier=-30, tag_type=TaskTagType.TIMEOUT_RESPONSE, is_active=True),
            TaskTag(name="延迟完成", work_minutes_modifier=-30, tag_type=TaskTagType.TIMEOUT_PROCESSING, is_active=True),
            TaskTag(name="差评", work_minutes_modifier=-60, tag_type=TaskTagType.BAD_RATING, is_active=True),
        ]
        ...
        total_minutes = task.calculate_work_minutes()
        assert total_minutes >= 0, f"Work minutes should not be negative, got {total_minutes}"