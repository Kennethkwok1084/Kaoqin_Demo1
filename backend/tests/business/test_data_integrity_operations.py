...
class TestBulkOperationIntegrity:
    ...
    async def test_bulk_import_data_validation(self, async_session):
        ...
        successful_tasks = await async_session.execute(
            select(RepairTask).where(RepairTask.task_id.in_(["BULK_VALID_001", "BULK_VALID_002"]))
        )
        ...
        failed_tasks = await async_session.execute(
            select(RepairTask).where(RepairTask.task_id.in_(["BULK_INVALID_001", "BULK_INVALID_002"]))
        )
    ...
    async def test_bulk_update_transaction_integrity(self, async_session):
        ...
        try:
            ...
            for task_id in task_ids[:3]:  # 前3个成功
                await async_session.execute(
                    text("UPDATE repair_tasks SET status = 'IN_PROGRESS' WHERE id = :task_id"),
                    {"task_id": task_id}
                )
            ...
            if len(task_ids) > 3:
                ...
                try:
                    await async_session.execute(
                        text("UPDATE repair_tasks SET status = 'INVALID_STATUS' WHERE id = :task_id"),
                        {"task_id": task_ids[3]}
                    )
                except Exception:
                    # 预期的失败，回滚事务
                    ...
                    for task in tasks:
                        ...
                        assert task.status == TaskStatus.PENDING  # 所有任务状态应该保持原始状态
    ...
    async def test_concurrent_operation_integrity(self, async_session):
        ...
        async def update_task_status(task_id: int, new_status: TaskStatus):
            ...
            try:
                await async_session.execute(
                    text("UPDATE repair_tasks SET status = :status WHERE id = :task_id"),
        ...
        # 最终状态应该是其中一个有效状态
        valid_statuses = [TaskStatus.IN_PROGRESS, TaskStatus.COMPLETED, TaskStatus.CANCELLED]
    ...
    async def test_data_consistency_after_rollback(self, async_session):
        ...
        initial_member_count = await async_session.scalar(
    ...
    async def test_cascade_delete_integrity(self, async_session):
        ...
        try:
            ...
            if remaining_task:
                ...
                assert remaining_task.member_id is None or remaining_task.member_id != member.id
