...
class TestTaskAssignmentWorkflow:
    ...
    async def test_task_status_transitions(self, async_session):
        """测试任务状态转换流程"""
        # 创建初始任务
        task, member, admin = await self.test_task_creation_and_assignment(async_session)
        service = TaskService(async_session)
    ...
    async def test_task_assignment_permissions(self, async_session):
        ...
        # 创建不同角色的用户
        admin = Member(username="admin", name="管理员", student_id="A001", class_name="管理", role=UserRole.ADMIN)
        leader = Member(username="leader", name="组长", student_id="L001", class_name="技术", role=UserRole.GROUP_LEADER)
        member1 = Member(username="member1", name="成员1", student_id="M001", class_name="技术", role=UserRole.MEMBER)
        member2 = Member(username="member2", name="成员2", student_id="M002", class_name="技术", role=UserRole.MEMBER)
        guest = Member(username="guest", name="访客", student_id="G001", class_name="访客", role=UserRole.GUEST)
    ...
    async def test_task_priority_handling(self, async_session):
        ...
        for i, priority in enumerate(priorities):
            task = RepairTask(
                ...
                priority=priority,
                member_id=member.id,
                report_time=datetime.utcnow() - timedelta(minutes=i*10)  # 不同报告时间
            )
            tasks.append(task)
    ...
    async def test_task_escalation_workflow(self, async_session):
        ...
        admin = Member(username="admin", name="管理员", student_id="A001", class_name="管理", role=UserRole.ADMIN)
        leader = Member(username="leader", name="组长", student_id="L001", class_name="技术", role=UserRole.GROUP_LEADER)  
        member = Member(username="member", name="成员", student_id="M001", class_name="技术", role=UserRole.MEMBER)
    ...
    async def test_task_cancellation_workflow(self, async_session):
        ...
        # 创建测试用户和任务
        admin = Member(username="cancel_admin", name="管理员", student_id="CA001", class_name="管理", role=UserRole.ADMIN)
        member = Member(username="cancel_member", name="成员", student_id="CM001", class_name="技术", role=UserRole.MEMBER)
