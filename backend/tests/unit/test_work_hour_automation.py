...
class TestWorkHourAutomationService:
    ...
    @pytest.mark.asyncio
    async def test_schedule_overdue_detection_success(self, automation_service, mock_db):
        """Test successful overdue detection scheduling."""
        # Mock the individual detection methods
        with patch.object(automation_service, '_detect_late_response_tasks', return_value=5), \
             patch.object(automation_service, '_detect_late_completion_tasks', return_value=3), \
             patch.object(automation_service, '_detect_long_overdue_tasks', return_value=2):
    ...
    @pytest.mark.asyncio
    async def test_detect_late_response_tasks(self, automation_service, mock_db, overdue_task):
    ...
    @pytest.mark.asyncio
    async def test_detect_late_completion_tasks(self, automation_service, mock_db, completed_task_late_response):
        """Test detection of late completion tasks."""
        # Mock database query to return late completion tasks
        mock_result = Mock()
        mock_result.scalars.return_value.all.return_value = [completed_task_late_response]
        mock_db.execute.return_value = mock_result
    ...
    @pytest.mark.asyncio
    async def test_apply_penalty_tag_success(self, automation_service, mock_db, overdue_task):
        ...
        mock_assoc_result.scalar_one_or_none.return_value = None  # No existing association
    ...
    @pytest.mark.asyncio
    async def test_apply_penalty_tag_already_exists(self, automation_service, mock_db, overdue_task):
    ...
    @pytest.mark.asyncio
    async def test_create_new_penalty_tag(self, automation_service, mock_db, overdue_task):
    ...
    @pytest.mark.asyncio
    async def test_schedule_evaluation_response_automation(self, automation_service, mock_db):
        ...
        with patch.object(automation_service, '_process_evaluation_response', return_value=True):
    ...
    @pytest.mark.asyncio
    async def test_process_evaluation_response_positive(self, automation_service, mock_db):
        ...
        with patch.object(automation_service, '_apply_bonus_tag', return_value=True):
            ...
            result = await automation_service._process_evaluation_response(excellent_task)
            
            assert result is True

    @pytest.mark.asyncio
    async def test_process_evaluation_response_negative(self, automation_service, mock_db):
    ...
    async def test_batch_recalculate_work_hours(self, automation_service, mock_db):
        ...
        with patch.object(automation_service.task_service, 'recalculate_task_work_hours', return_value=True):
    ...
    async def test_error_handling_in_detection(self, automation_service, mock_db):
        ...
        with patch.object(automation_service, '_detect_late_response_tasks', side_effect=Exception("Detection error")):
    ...
    async def test_tag_validation(self, automation_service):
        ...
        invalid_names = ["", None, "   ", "a" * 101]  # Empty, None, whitespace, too long
    ...
    async def test_time_threshold_calculations(self, automation_service):
        ...
        urgent_threshold = automation_service._get_response_time_threshold(is_urgent=True)
        normal_threshold = automation_service._get_response_time_threshold(is_urgent=False)
        ...
        urgent_completion = automation_service._get_completion_time_threshold(is_urgent=True)
        normal_completion = automation_service._get_completion_time_threshold(is_urgent=False)
    ...
    async def test_concurrent_processing_safety(self, automation_service, mock_db):
        ...
        with patch.object(automation_service, '_acquire_processing_lock', return_value=True):
        ...
        with patch.object(automation_service, '_acquire_processing_lock', return_value=False):
    ...
    def test_helper_methods(self, automation_service):
        ...
        assert automation_service._get_response_time_threshold(True) == timedelta(minutes=30)
        assert automation_service._get_completion_time_threshold(False) == timedelta(hours=72)