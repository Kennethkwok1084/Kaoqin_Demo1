...
class TestTaskServiceComprehensive:
    ...
    async def test_create_task_with_validation(self, task_service: TaskService, sample_member):
        """测试任务创建业务逻辑验证"""
        task_data = {
            "title": "新建测试任务",
            "description": "任务详细描述",
    ...
    async def test_assign_task_business_logic(self, task_service: TaskService, sample_task, sample_member):
        """测试任务分配业务逻辑"""
        # Mock数据库查询
    ...
    async def test_update_task_status_workflow(self, task_service: TaskService, sample_task):
        """测试任务状态更新工作流"""
        # Mock任务查询
        task_service.session.get.return_value = sample_task
        
        # 测试状态转换：PENDING -> IN_PROGRESS
        await task_service.update_task_status(sample_task.id, TaskStatus.IN_PROGRESS, "开始处理")
    ...
    async def test_calculate_work_hours_logic(self, task_service: TaskService, sample_task):
        """测试工时计算核心逻辑"""
        # Mock任务查询
        task_service.session.get.return_value = sample_task
        ...
        with patch('app.services.task_service.WorkHoursService') as mock_work_hours:
            ...
            mock_work_hours_instance.calculate_hours = AsyncMock(return_value={
                "calculated_hours": 2.5,
                "bonus_hours": 0.5,
                "total_hours": 3.0
            })
            
            result = await task_service.calculate_work_hours(sample_task.id, calculation_data)
            
            # 验证工时计算调用
            mock_work_hours_instance.calculate_hours.assert_called_once()
            assert result["total_hours"] == 3.0

    async def test_batch_operation_processing(self, task_service: TaskService, sample_member):
        """测试批量操作处理逻辑"""
        task_ids = [1, 2, 3, 4, 5]
        operation_data = {
        ...
        # 调用批量操作
        result = await task_service.batch_update_tasks(task_ids, operation_data, sample_member.id)
    ...
    async def test_task_validation_rules(self, task_service: TaskService, sample_member):
        """测试任务验证规则"""
        # 测试无效数据验证
        invalid_task_data = {
            "title": "",  # 空标题
    ...
    async def test_task_priority_management(self, task_service: TaskService, sample_task):
        """测试任务优先级管理逻辑"""
        # Mock任务查询
        task_service.session.get.return_value = sample_task
        
        # 测试优先级提升
        await task_service.escalate_task_priority(sample_task.id, "紧急情况需要提高优先级")
    ...
    async def test_task_dependency_management(self, task_service: TaskService):
        ...
        child_tasks = [Mock(id=tid, status=TaskStatus.PENDING) for tid in child_task_ids]
        
        task_service.session.get.side_effect = [parent_task] + child_tasks
        
        result = await task_service.create_task_dependencies(parent_task_id, child_task_ids)
    ...
    async def test_task_template_management(self, task_service: TaskService, sample_member):
        """测试任务模板管理逻辑"""
        template_data = {
            "name": "网络维修标准模板",
        ...
        # 创建任务模板
        result = await task_service.create_task_template(template_data, sample_member.id)
    ...
    async def test_task_notification_logic(self, task_service: TaskService, sample_task):
        """测试任务通知逻辑"""
        # Mock任务查询
        task_service.session.get.return_value = sample_task
        ...
        with patch('app.services.task_service.NotificationService') as mock_notification:
            ...
            result = await task_service.send_task_notification(sample_task.id, notification_data)
            
            # 验证通知发送
            mock_notification_instance.send_notification.assert_called_once()
            assert result["sent_count"] >= 0

    async def test_task_audit_logging(self, task_service: TaskService, sample_task, sample_member):
        """测试任务审计日志记录"""
        # Mock任务查询
        task_service.session.get.return_value = sample_task
        ...
        result = await task_service.log_task_audit(sample_task.id, sample_member.id, audit_data)
