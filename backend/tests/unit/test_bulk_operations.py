...
from app.core.exceptions import OperationTimeoutError, ValidationError
...
class TestLargeScaleBulkOperations:
    ...
    async def test_bulk_rush_task_marking_large_scale(
        ...
        for i in range(0, len(large_task_ids), batch_size):
            ...
            if batch_failed > 0:
                errors.extend(
                    [f"任务ID{tid}处理失败" for tid in batch_ids[-batch_failed:]]
                )
    ...
    async def test_bulk_work_hours_recalculation_large_scale(
        ...
        with patch.object(
            ...
            for i in range(processed_count):
                ...
                results.append(
                    {"member_id": member_id, "status": "success", **calc_result}
                )
    ...
    @pytest.mark.asyncio
    async def test_bulk_data_import_large_dataset(self, import_service, mock_db):
        """测试大数据集批量导入"""
        # 创建50000条导入数据
        large_import_data = [
            {
        ...
        with (
            ...
            patch.object(
                import_service, "process_repair_tasks_import"
    ...
    async def test_memory_usage_optimization_bulk_operations(
        ...
        for dataset in large_datasets:
            ...
            with (
                ...
                assert (
                    operation_time < max_expected_time
...
class TestBulkOperationErrorHandling:
    ...
    async def test_partial_failure_handling_bulk_rush_marking(
        ...
        with patch.object(task_service, "_bulk_add_rush_tags") as mock_bulk_tag:
            mock_bulk_tag.return_value = {
                "success": 2,
                "failed": 3,
                "errors": ["任务3状态不允许标记爆单", "任务4不存在", "任务5不存在"],
            }

            result = await task_service.bulk_mark_rush_tasks(
                mixed_task_ids, admin_user.id
            )
    ...
    async def test_data_integrity_bulk_operations(self, import_service, mock_db):
        ...
        with patch.object(import_service, "_validate_import_data") as mock_validate:
