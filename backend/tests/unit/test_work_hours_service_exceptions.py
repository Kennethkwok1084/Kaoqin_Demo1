...
class TestWorkHoursServiceExceptions:
    ...
    @pytest.mark.asyncio
    async def test_calculate_task_work_minutes_invalid_task_id(self, work_hours_service):
    ...
    @pytest.mark.asyncio
    async def test_calculate_task_work_minutes_invalid_task_type(self, work_hours_service):
    ...
    @pytest.mark.asyncio
    async def test_calculate_task_work_minutes_invalid_report_time(self, work_hours_service):
    ...
    @pytest.mark.asyncio
    async def test_calculate_monthly_work_hours_invalid_member_id(self, work_hours_service):
    ...
    @pytest.mark.asyncio
    async def test_calculate_monthly_work_hours_member_not_found(self, work_hours_service):
    ...
    @pytest.mark.asyncio
    async def test_batch_update_monthly_summaries_invalid_year(self, work_hours_service):
        """Test batch_update_monthly_summaries with invalid year."""
        with pytest.raises(ValueError, match="年份必须在2020-2050范围内"):
            await work_hours_service.batch_update_monthly_summaries(2019, 1)
    
    @pytest.mark.asyncio
    async def test_batch_update_monthly_summaries_invalid_member_ids(self, work_hours_service):
        """Test batch_update_monthly_summaries with invalid member IDs."""
        with pytest.raises(ValueError, match="成员ID列表必须为列表类型"):
            await work_hours_service.batch_update_monthly_summaries(2024, 1, "not_a_list")
        
        with pytest.raises(ValueError, match="成员ID必须为正整数"):
            await work_hours_service.batch_update_monthly_summaries(2024, 1, [1, -1, 3])
        
        with pytest.raises(ValueError, match="成员ID必须为正整数"):
            await work_hours_service.batch_update_monthly_summaries(2024, 1, [1, "invalid", 3])
...
class TestRushTaskMarkingServiceExceptions:
    ...
    async def test_mark_rush_tasks_invalid_task_ids(self, rush_service):
        ...
        with pytest.raises(ValueError, match="任务ID必须为正整数"):
            await rush_service.mark_rush_tasks_by_date(date_from, date_to, [1, "invalid", 3])
    ...
    async def test_mark_rush_tasks_invalid_marked_by(self, rush_service):
        ...
        with pytest.raises(ValueError, match="标记人ID必须为正整数"):
            await rush_service.mark_rush_tasks_by_date(date_from, date_to, marked_by="invalid")
    ...
    async def test_batch_recalculate_invalid_date_types(self, rush_service):
        ...
        with pytest.raises(ValueError, match="开始日期必须为date对象"):
            await rush_service.batch_recalculate_work_hours("2024-01-01", date(2024, 1, 31))
        
        with pytest.raises(ValueError, match="结束日期必须为date对象"):
            await rush_service.batch_recalculate_work_hours(date(2024, 1, 1), "2024-01-31")
    ...
    async def test_batch_recalculate_invalid_member_ids(self, rush_service):
        ...
        with pytest.raises(ValueError, match="成员ID列表必须为列表类型"):
            await rush_service.batch_recalculate_work_hours(date_from, date_to, "not_a_list")
        
        with pytest.raises(ValueError, match="成员ID必须为正整数"):
            await rush_service.batch_recalculate_work_hours(date_from, date_to, [1, -1, 3])
        
        with pytest.raises(ValueError, match="成员ID必须为正整数"):
            await rush_service.batch_recalculate_work_hours(date_from, date_to, [1, "invalid", 3])
...
class TestServiceRuntimeExceptions:
    ...
    async def test_calculate_task_work_minutes_runtime_error(self, work_hours_service):
        ...
        work_hours_service.db.refresh.side_effect = Exception("Database connection lost")
