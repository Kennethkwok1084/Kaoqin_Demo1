...
class TestStatisticsService:
    ...
    @pytest.mark.asyncio
    async def test_get_overview_statistics_basic(self, stats_service, mock_db, sample_tasks):
    ...
    async def test_get_system_overview_zero_tasks(self, stats_service, mock_db):
        ...
        with patch.object(stats_service, '_get_performance_metrics', return_value={
    ...
    @pytest.mark.asyncio
    async def test_get_member_performance_stats(self, stats_service, mock_db, sample_member):
    ...
    async def test_get_workload_distribution(self, stats_service, mock_db):
        ...
        mock_result = Mock()
        mock_result.all.return_value = [
            (1, "张三", "TEST001", 25, 23, 920),  # member_id, name, student_id, total, completed, work_minutes
    ...
    async def test_caching_functionality(self, stats_service, mock_db):
        ...
        with patch.object(cache, 'get_stats_cache', return_value={"data": cached_data}):
            with patch.object(stats_service, '_generate_cache_key', return_value=cache_key):
    ...
    async def test_get_efficiency_analysis(self, stats_service, mock_db):
        ...
        mock_result = Mock()
        mock_result.all.return_value = [
            (1, "张三", 2.5, 4.8, 8.5),  # member_id, name, avg_completion_hours, avg_rating, efficiency_score
            (2, "李四", 3.0, 4.5, 7.8),
            (3, "王五", 1.8, 4.9, 9.2)
        ]
        mock_db.execute.return_value = mock_result
