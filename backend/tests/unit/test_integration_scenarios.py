...
class TestEndToEndBusinessFlows:
    ...
    async def test_member_performance_evaluation_flow(
        ...
        with patch.object(
            ...
            return_value={"total_tasks": 25, "completed_tasks": 24, "completion_rate": 96.0}
        ), patch.object(
            stats_service,
            ...
            return_value={"avg_rating": 4.5, "positive_rate": 92.0, "overdue_response": 1}
        ), patch.object(
            stats_service,
            '_get_member_ranking',
            return_value={"work_hour_rank": 3, "total_members": 10, "percentile": 70.0}
        ), patch.object(
            stats_service,
...
class TestServiceInteractionIntegration:
    ...
    async def test_attendance_work_hours_integration(
        ...
        attendance_record = AttendanceRecord(
            ...
            checkin_time=datetime.combine(attendance_date, datetime.min.time().replace(hour=8)),
            checkout_time=datetime.combine(attendance_date, datetime.min.time().replace(hour=18)),
            work_hours=8.0,
        ...
        with patch.object(attendance_service, 'get_attendance_by_date', return_value=attendance_record):
    ...
    async def test_task_statistics_data_consistency(
        ...
        expected_completed = len([t for t in mock_tasks if t.status == TaskStatus.COMPLETED])
        ...
        assert stats_result["completion_rate"] == round(expected_completed / expected_total * 100, 2)
