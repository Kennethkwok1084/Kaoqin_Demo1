...
class TestTasksAPIComprehensive:
    ...
    async def sample_tasks(
        self, async_session: AsyncSession, admin_user: Member, regular_user: Member
    ):
        """创建示例任务数据"""
        tasks = []
    ...
    async def test_get_tasks_status_overview(
        self, async_client: AsyncClient, sample_tasks, admin_user: Member
    ):
        """测试任务状态概览 - 覆盖状态统计逻辑"""
        headers = await self.get_auth_headers(
            async_client, "ADMIN_TASKS_001", "AdminPass123!"
        )
    ...
    async def test_get_monitoring_tasks(
        self, async_client: AsyncClient, sample_tasks, admin_user: Member
    ):
        ...
        data = response.json()
        monitoring_tasks = [
            t for t in data["data"]["tasks"] if t["task_type"] == "MONITORING"
        ]
        assert len(monitoring_tasks) >= 0

    async def test_create_repair_task_complete_flow(
    ...
    async def test_update_repair_task_comprehensive(
        self, async_client: AsyncClient, sample_tasks, admin_user: Member
    ):
        """测试更新维修任务 - 覆盖任务更新逻辑"""
        headers = await self.get_auth_headers(
        ...
        response = await async_client.put(
            f"/api/v1/tasks/repair/{task_id}", json=update_data, headers=headers
        )
        assert response.status_code == 200
    ...
    async def test_task_status_workflow(
        self,
        ...
        """测试任务状态工作流 - 覆盖状态转换逻辑"""
        admin_headers = await self.get_auth_headers(
            async_client, "ADMIN_TASKS_001", "AdminPass123!"
        )
        user_headers = await self.get_auth_headers(
            async_client, "USER_TASKS_001", "UserPass123!"
        )
        ...
        # 2. 开始任务
        response = await async_client.post(
            f"/api/v1/tasks/{task_id}/start", headers=user_headers
        )
        assert response.status_code == 200
    ...
    async def test_batch_operations(
        ...
        response = await async_client.delete(
            "/api/v1/tasks/batch-delete", json=batch_data, headers=headers
        )
        assert response.status_code == 200
    ...
    async def test_task_statistics_comprehensive(
        self, async_client: AsyncClient, sample_tasks, admin_user: Member
    ):
        """测试任务统计 - 覆盖统计分析逻辑"""
        headers = await self.get_auth_headers(
    ...
    async def test_work_hours_bulk_recalculate(
        self, async_client: AsyncClient, sample_tasks, admin_user: Member
    ):
        """测试批量工时重算 - 覆盖批量计算逻辑"""
        headers = await self.get_auth_headers(
    ...
    async def test_task_tags_management(
        ...
        response = await async_client.post(
            "/api/v1/tasks/tags", json=tag_data, headers=headers
        )
        assert response.status_code == 200
    ...
    async def test_my_tasks_with_filters(
        self, async_client: AsyncClient, sample_tasks, regular_user: Member
    ):
        """测试我的任务查询 - 覆盖个人任务过滤逻辑"""
        headers = await self.get_auth_headers(
            async_client, "USER_TASKS_001", "UserPass123!"
        ...
        response = await async_client.get(
            "/api/v1/tasks/my", params=filter_params, headers=headers
        )
        assert response.status_code == 200
    ...
    async def test_monitoring_inspection_workflow(
        self, async_client: AsyncClient, admin_user: Member
    ):
        """测试监控任务检查工作流 - 覆盖监控逻辑"""
        headers = await self.get_auth_headers(
        ...
        response = await async_client.post(
            "/api/v1/tasks/monitoring", json=task_data, headers=headers
        )
        assert response.status_code == 200
        ...
        response = await async_client.put(
            f"/api/v1/tasks/monitoring/{task_id}/inspection",
        ...
        # 3. 完成检查
        response = await async_client.put(
            f"/api/v1/tasks/monitoring/{task_id}/inspection/complete", headers=headers
        )
        assert response.status_code == 200
    ...
    async def test_assistance_approval_workflow(
        self, async_client: AsyncClient, admin_user: Member, regular_user: Member
    ):
        """测试协助任务审批工作流 - 覆盖审批逻辑"""
        admin_headers = await self.get_auth_headers(
            async_client, "ADMIN_TASKS_001", "AdminPass123!"
        ...
        response = await async_client.post(
            "/api/v1/tasks/assistance", json=task_data, headers=user_headers
        )
        assert response.status_code == 200
        ...
        response = await async_client.post(
            f"/api/v1/tasks/assistance/{task_id}/review",
            json=review_data,
            headers=user_headers,
        )
        assert response.status_code == 200
