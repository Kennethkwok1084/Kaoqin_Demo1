...
from app.models.task import (
    RepairTask, TaskType, TaskStatus, TaskCategory, 
    TaskPriority, TaskTag, TaskTagType, MonitoringTask, AssistanceTask
)
...
class TestTaskServiceFixed:
    ...
    async def test_create_repair_task_basic(self, async_session):
        ...
        with patch.object(service, '_generate_task_id', return_value='T202501270001'):
            ...
            with patch.object(service, '_add_tags_to_task', return_value=None), \
    ...
    async def test_create_repair_task_with_tags(self, async_session):
        ...
        with patch.object(service, '_generate_task_id', return_value='T202501270002'), \
             patch.object(service, '_add_tags_to_task', return_value=None) as mock_add_tags, \
             patch.object(service, '_add_rush_tag', return_value=None), \
             patch.object(service, '_auto_add_penalty_tags', return_value=None), \
             patch.object(service, '_auto_add_bonus_tags', return_value=None):
    ...
    async def test_create_monitoring_task_success(self, async_session):
        ...
        with patch.object(service.work_hours_service, 'update_monthly_summary', return_value=None):
    ...
    async def test_update_task_status_success(self, async_session):
        ...
        mock_task = RepairTask(
            ...
            status=TaskStatus.PENDING, member_id=1
        )
        ...
        with patch.object(async_session, 'execute', return_value=mock_result), \
             patch.object(service, '_recalculate_work_hours_if_needed', return_value=None):
            async_session.commit = AsyncMock()
    ...
    async def test_assign_task_success(self, async_session):
        ...
        with patch.object(async_session, 'execute', side_effect=[mock_result, mock_member_result]):
            async_session.commit = AsyncMock()
    ...
    async def test_assign_task_member_not_found(self, async_session):
        ...
        with patch.object(async_session, 'execute', side_effect=[mock_task_result, mock_member_result]):
            async_session.rollback = AsyncMock()
    ...
    async def test_add_task_feedback_positive(self, async_session):
        ...
        mock_task = RepairTask(
            id=1, task_id='T001', title='Test Task',
            status=TaskStatus.COMPLETED, rating=None
        )
        mock_result = Mock()
        mock_result.scalar_one_or_none.return_value = mock_task
    ...
    async def test_get_member_task_summary(self, async_session):
        ...
        with patch.object(service, '_calculate_member_summary', return_value=mock_summary_data):
    ...
    async def test_batch_mark_rush_tasks(self, async_session):
        ...
        with patch.object(service.rush_task_service, 'batch_mark_rush_tasks', return_value=3) as mock_batch:
    ...
    async def test_get_rush_tasks_list(self, async_session):
        ...
        with patch.object(service.rush_task_service, 'get_rush_tasks_list', return_value=mock_tasks):
    ...
    async def test_get_rush_tasks_statistics(self, async_session):
        ...
        with patch.object(service.rush_task_service, 'get_statistics', return_value=mock_stats):
    ...
    async def test_remove_rush_task_marking(self, async_session):
        ...
        with patch.object(service.rush_task_service, 'remove_rush_marking', return_value=True) as mock_remove:
    ...
    @pytest.mark.parametrize("task_type,expected_minutes", [
        (TaskType.ONLINE, 40),
        (TaskType.OFFLINE, 100)
    ])
    ...
    async def test_error_handling_database_error(self, async_session):
        ...
        async_session.add = Mock()
        async_session.flush = AsyncMock(side_effect=Exception("Database connection error"))
        async_session.rollback = AsyncMock()
