...
class TestConcurrentRushTaskMarking:
    ...
    @pytest.mark.asyncio
    async def test_concurrent_rush_marking_same_tasks(
        ...
        async def simulate_rush_marking(user: Member, delay: float = 0):
            ...
            # Mock 任务查询
            available_tasks = [
                task for task in shared_tasks 
                if task.id not in marked_tasks
            ]
            ...
            if available_tasks:
                # 标记成功的任务
                successful_ids = [task.id for task in available_tasks[:3]]  # 最多标记3个
        ...
        # 验证并发冲突处理
        total_success = sum(result["success"] for result in results if not isinstance(result, Exception))
        total_failed = sum(result["failed"] for result in results if not isinstance(result, Exception))
        ...
        if not isinstance(user1_result, Exception) and not isinstance(user2_result, Exception):
            # 至少有一个用户应该遇到冲突
            assert user1_result["failed"] > 0 or user2_result["failed"] > 0
            
            # 验证错误信息包含冲突提示
            all_errors = user1_result["errors"] + user2_result["errors"]
            conflict_errors = [error for error in all_errors if "已被" in error or "冲突" in error]
            assert len(conflict_errors) > 0

    @pytest.mark.asyncio
    async def test_high_concurrency_rush_marking(
        self, task_service, mock_db
    ):
        """测试高并发爆单标记场景"""
        # 大量任务和用户
        ...
        async def concurrent_marking_operation(user: Member):
            ...
            async with lock:
                # 检查哪些任务还未被标记
                available_tasks = [
                    task_id for task_id in selected_tasks
        ...
        concurrent_tasks = [
            concurrent_marking_operation(user) 
            for user in users
        ]
        ...
        logger.info(f"高并发测试: {user_count}用户, {task_count}任务, "
...
class TestConcurrentWorkHoursCalculation:
    ...
    async def test_concurrent_monthly_calculation_same_member(
        ...
        mock_db.execute.side_effect = [
            mock_tasks_result, mock_empty_result, mock_empty_result,  # 第1个请求
            mock_tasks_result, mock_empty_result, mock_empty_result,  # 第2个请求
            mock_tasks_result, mock_empty_result, mock_empty_result,  # 第3个请求
            mock_tasks_result, mock_empty_result, mock_empty_result,  # 第4个请求
            mock_tasks_result, mock_empty_result, mock_empty_result,  # 第5个请求
        ]
        ...
        if successful_results:
            ...
            for result in successful_results[1:]:
                ...
                assert result["repair_task_hours"] == expected_result["repair_task_hours"]
    ...
    @pytest.mark.asyncio
    async def test_concurrent_bulk_recalculation(
        self, work_hours_service, mock_db
    ):
        """测试批量工时重算的并发场景"""
        # 多个用户同时触发批量重算
        ...
        async def batch_recalculation(batch_members: List[int], batch_id: int):
            ...
            return {
                "batch_id": batch_id,
                "processed": len([r for r in batch_results if r["status"] == "success"]),
                "conflicts": len([r for r in batch_results if r["status"] == "conflict"]),
        ...
        logger.info(f"并发批量重算: 处理{total_processed}成员, "
...
class TestResourceContention:
    ...
    async def test_database_connection_pool_stress(
        ...
        async def database_query_simulation(query_id: int):
            ...
            # 模拟查询延迟
            base_delay = 0.01
            contention_delay = query_id * 0.001  # 随着并发增加，延迟增加
        ...
        assert performance_degradation < 2.0, f"性能退化{performance_degradation:.1%}过大"

    @pytest.mark.asyncio
    async def test_memory_pressure_under_load(
        self, stats_service, mock_db
    ):
        """测试高负载下的内存压力"""
        # 模拟大数据量处理
        ...
        for i in range(20):  # 20个大数据操作
            ...
            large_dataset = {
                ...
                'tasks': [
                    {'id': j, 'status': 'completed', 'duration': random.uniform(30, 180)}
        ...
        async def process_large_dataset(dataset):
            ...
            avg_duration = sum(task['duration'] for task in dataset['tasks']) / task_count
        ...
        processing_tasks = [
            ...
            for dataset in large_data_operations
        ]
        ...
        processing_rate = (total_members_processed + total_tasks_processed) / total_processing_time
        assert processing_rate > 10000, f"处理速度{processing_rate:.0f}/秒过慢"
        
        logger.info(f"大数据处理测试: 处理{len(successful_operations)}个数据集, "
...
class TestLongRunningStability:
    ...
    async def test_extended_operation_stability(
        ...
        while time.time() - start_time < test_duration:
            try:
                ...
                with patch.object(
                    ...
                    await asyncio.gather(*operations[:1])  # 只执行第一个操作避免复杂mock
                ...
                if operation_count % 10 == 0:
                    ...
                    if recent_avg > 1.0:  # 响应时间超过1秒
                        logger.warning(f"操作{operation_count}: 响应时间退化至{recent_avg:.2f}秒")
        ...
        assert error_count <= operation_count * 0.05, f"错误率过高: {error_count}/{operation_count}"
        ...
        logger.info(f"长时间稳定性测试: 运行{actual_duration:.1f}秒, "
    ...
    async def test_error_recovery_resilience(
        ...
        for error_type, error_message in error_scenarios:
            try:
                ...
                if error_type == "database_timeout":
                    with patch.object(mock_db, 'execute', side_effect=Exception(error_message)):
                    ...
                    with patch.object(
                        ...
                        await work_hours_service.calculate_monthly_work_hours(1, 2024, 3)
        ...
        logger.info(f"错误恢复测试: {len(error_scenarios)}种错误场景, "
