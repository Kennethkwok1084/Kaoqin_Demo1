...
class TestExtremeDataHandling:
    ...
    @pytest.mark.asyncio
    async def test_massive_task_volume_statistics(self, stats_service, mock_db):
        """测试大量任务的统计计算"""
        ...
        result = await stats_service._get_monthly_statistics(date_from, date_to)
    ...
    @pytest.mark.asyncio
    async def test_zero_work_hours_edge_case(self, stats_service, mock_db):
        """测试零工时的边缘情况"""
        date_from = datetime(2024, 3, 1)
        date_to = datetime(2024, 3, 31)
    ...
    @pytest.mark.asyncio
    async def test_maximum_rating_boundary(self, stats_service, mock_db):
        """测试最大评分边界处理"""
        date_from = datetime(2024, 3, 1)
        date_to = datetime(2024, 3, 31)
...
class TestInvalidInputHandling:
    ...
    @pytest.mark.asyncio
    async def test_invalid_date_range(self, stats_service, mock_db):
        """测试无效日期范围"""
        # 结束日期早于开始日期
    ...
    @pytest.mark.asyncio
    async def test_negative_member_id(self, work_hours_service, mock_db):
        """测试负数成员ID"""
        invalid_member_id = -1
        year, month = 2024, 3
    ...
    @pytest.mark.asyncio
    async def test_future_date_statistics(self, stats_service, mock_db):
        """测试未来日期统计"""
        # 设置未来日期
...
class TestDatabaseErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_connection_timeout_recovery(self, stats_service, mock_db):
        """测试连接超时恢复"""
        date_from = datetime(2024, 3, 1)
        date_to = datetime(2024, 3, 31)
    ...
    @pytest.mark.asyncio
    async def test_integrity_constraint_violation(self, stats_service, mock_db):
        """测试完整性约束违反"""
        date_from = datetime(2024, 3, 1)
        date_to = datetime(2024, 3, 31)
        ...
        with pytest.raises(IntegrityError):
            await stats_service._get_performance_statistics_cached(date_from, date_to)

    @pytest.mark.asyncio
    async def test_concurrent_statistics_calculation(self, stats_service, mock_db):
        """测试并发统计计算"""
        date_from = datetime(2024, 3, 1)
        date_to = datetime(2024, 3, 31)
...
class TestDataConsistencyValidation:
    ...
    @pytest.mark.asyncio
    async def test_work_hours_consistency_check(self, stats_service, mock_db):
        """测试工时数据一致性"""
        year, month = 2024, 3
    ...
    @pytest.mark.asyncio
    async def test_rating_data_validation(self, stats_service, mock_db):
        """测试评分数据有效性"""
        date_from = datetime(2024, 3, 1)
        date_to = datetime(2024, 3, 31)
    ...
    @pytest.mark.asyncio
    async def test_attendance_record_completeness(self, stats_service, mock_db):
        """测试考勤记录完整性"""
        date_from = datetime(2024, 3, 1)
        date_to = datetime(2024, 3, 31)
    ...
    @pytest.mark.asyncio
    async def test_cross_month_boundary_statistics(self, stats_service, mock_db):
        """测试跨月边界统计"""
        # 测试月初和月末边界
        boundary_cases = [
            # 2月末到3月初（包含闰年检查）
...
class TestMemoryAndPerformance:
    ...
    @pytest.mark.asyncio
    async def test_large_member_list_processing(self, stats_service, mock_db):
        """测试大量成员列表处理"""
        # 模拟1000个成员
        large_member_list = [
            Member(
    ...
    @pytest.mark.asyncio
    async def test_cache_performance_under_load(self, stats_service, mock_db):
        """测试高负载下的缓存性能"""
        date_from = datetime(2024, 3, 1)
        date_to = datetime(2024, 3, 31)
