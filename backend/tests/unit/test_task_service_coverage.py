...
class TestTaskService:
    ...
    async def test_create_repair_task_rush_order(self, async_session, sample_member):
        ...
        with patch.object(service, '_generate_task_id', return_value='T202501270003'), \
    ...
    async def test_get_overdue_tasks(self, async_session):
        ...
        mock_tasks = [
            RepairTask(
                id=1, task_id='T001', title='Overdue Task 1',
                status=TaskStatus.PENDING, report_time=old_time
            ),
            RepairTask(
                id=2, task_id='T002', title='Overdue Task 2', 
                status=TaskStatus.IN_PROGRESS, response_time=old_time
            )
        ]
