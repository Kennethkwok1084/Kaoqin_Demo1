...
class TestTaskAttendanceConsistency:
    ...
    async def test_task_attendance_time_consistency(
        ...
        with patch.object(
            ...
            return_value=attendance_record
        ):
    ...
    async def test_cross_date_consistency_validation(
        ...
        # 验证跨服务数据一致性
        total_attendance_hours = sum(att.work_hours for att in attendance_data)  # 56小时
        total_task_hours = sum(task.work_minutes for task in task_data) / 60     # 25.69小时
        calculated_work_hours = work_hours_result['total_hours']                 # 25.69小时
        
        assert calculated_work_hours == round(total_task_hours, 2)
...
class TestWorkHoursStatisticsConsistency:
    ...
    async def test_monthly_summary_consistency(
        ...
        members_data = [
            {
                'id': 1, 'hours': 35.5, 'tasks': 20,
                'name': '高效成员', 'attendance_rate': 100.0
            },
            {
                'id': 2, 'hours': 28.0, 'tasks': 16,
                'name': '标准成员', 'attendance_rate': 95.0
            },
            {
                'id': 3, 'hours': 22.5, 'tasks': 12,
                'name': '新成员', 'attendance_rate': 90.0
            },
        ]
        ...
        with patch.object(
            ...
            return_value={
                ...
                'average_hours_per_member': round(total_calculated_hours / len(members_data), 2),
        ...
        assert work_hours_summary['average_hours_per_member'] == stats_summary['avg_work_hours']
        assert work_hours_summary['total_tasks'] == stats_summary['total_completed_tasks']
...
class TestLongRunningStabilityTests:
    ...
    async def test_continuous_operation_stability(
        ...
        assert operation_count >= expected_operations * 0.9, f"操作次数不足: {operation_count}"
...
class TestServiceIntegrationErrorHandling:
    ...
    async def test_data_consistency_recovery(
        ...
        with patch.object(
            ...
            try:
                ...
                # 统计服务的数据应该是一致的
                stats_total = stats_result['repair_hours'] + stats_result['monitoring_hours']
    ...
    async def test_transaction_consistency_across_services(
        ...
        # Step 2: 重新计算工时
        with patch.object(work_hours_service, 'recalculate_member_hours') as mock_recalc:
            updated_hours = {
                'member_id': member_id,
