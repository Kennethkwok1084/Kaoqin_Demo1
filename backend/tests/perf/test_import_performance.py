...
from app.models.task import RepairTask, TaskType, TaskStatus, TaskPriority, TaskCategory
...
class TestImportPerformance:
    ...
    async def test_import_1000_records_performance(
        ...
        async def import_data():
            ...
            result = await import_service._match_ab_tables(
                sample_import_data_1000,
                {"type": "task_table", "columns": list(sample_import_data_1000[0].keys())}
            )
            return result
        ...
        assert benchmark_result['mean'] < 30.0, f"Import took {benchmark_result['mean']:.2f}s, should be < 30s"
    ...
    async def test_single_work_hour_calculation_performance(
        ...
        # Performance assertions
        benchmark_result = benchmark.stats
        assert benchmark_result['mean'] < 0.1, f"Calculation took {benchmark_result['mean']:.4f}s, should be < 0.1s"
    ...
    @pytest.mark.asyncio
    @pytest.mark.benchmark
    async def test_batch_work_hour_calculation_performance(
        self,
        benchmark,
        db_session
    ):
        ...
        # Performance assertions
        benchmark_result = benchmark.stats
        assert benchmark_result['mean'] < 10.0, f"Batch calculation took {benchmark_result['mean']:.2f}s, should be < 10s"
    ...
    @pytest.mark.asyncio
    @pytest.mark.benchmark  
    async def test_fuzzy_member_matching_performance(
        self,
        benchmark,
        db_session
    ):
        ...
        test_names = [
            "张三(信息处)", "李四（技术部）", "王五[维修组]", "赵六{网络组}", 
            "张三工程师", "李四技术员", "王五主管", "赵六组长"
        ...
        # Performance assertions
        benchmark_result = benchmark.stats
        assert benchmark_result['mean'] < 5.0, f"Fuzzy matching took {benchmark_result['mean']:.3f}s, should be < 5s"
