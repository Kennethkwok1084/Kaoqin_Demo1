...
class TestJWTTokens:
    ...
    def test_create_access_token_custom_expiry(self):
        ...
        assert (
            abs(
                datetime.fromtimestamp(exp_timestamp).timestamp()
    ...
    def test_verify_token_invalid_signature(self):
        ...
        invalid_token = jwt.encode(
            to_encode, "wrong_secret_key", algorithm=settings.ALGORITHM
        )
...
class TestSecurityHeaders:
    ...
    def test_get_security_headers_enabled(self):
        """测试安全头启用时"""
        with (
            patch.object(settings, "SECURITY_HEADERS_ENABLED", True),
