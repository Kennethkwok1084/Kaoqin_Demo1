...
class TestDatabaseDependencies:
    ...
    async def test_get_db_connection_error(self):
        ...
        with patch("app.api.deps.get_async_session") as mock_get_async:
            ...
            assert "未知消息" in str(exc_info.value.detail) or "连接" in str(
...
class TestQueryParameters:
    ...
    def test_common_query_params_custom(self):
        """测试自定义查询参数"""
        params = CommonQueryParams(
            page=2, size=50, search="test", sort_by="name", sort_order="asc"
        )
    ...
    def test_member_query_params_extended(self):
        ...
        params = MemberQueryParams(
            page=2, size=30, role="admin", group_id=5, is_active=True
        )
    ...
    def test_get_query_params_dependency(self):
        ...
        params = get_query_params(
            page=2, size=15, search="keyword", sort_by="created_at", sort_order="asc"
        )
    ...
    def test_get_task_query_params_dependency(self):
        """测试任务查询参数依赖函数"""
        params = get_task_query_params(page=1, status="pending", member_id=456)
    ...
    def test_get_member_query_params_dependency(self):
        """测试成员查询参数依赖函数"""
        params = get_member_query_params(role="member", is_active=False)
...
class TestResponseHelpers:
    ...
    def test_create_response_basic(self):
        ...
        response = create_response(
            data={"key": "value"}, message="操作成功", status_code=200
        )
    ...
    def test_create_response_with_message_key(self):
        ...
        with patch("app.api.deps.get_message") as mock_get_message:
            ...
            response = create_response(message_key="SUCCESS_MESSAGE", user_name="张三")

            assert response["message"] == "从消息键获取的消息"
            mock_get_message.assert_called_once_with(
                "SUCCESS_MESSAGE", user_name="张三"
            )
    ...
    def test_create_response_error_status_code(self):
        """测试错误状态码的成功值自动计算"""
        response = create_response(message="操作失败", status_code=400)
    ...
    def test_create_response_explicit_success(self):
        """测试显式设置成功值"""
        response = create_response(
            message="部分成功", status_code=200, success=False  # 显式设置
        )
    ...
    def test_create_error_response_basic(self):
        """测试基本错误响应创建"""
        response = create_error_response(message="操作失败", status_code=400)
    ...
    def test_create_error_response_with_message_key(self):
        ...
        with patch("app.api.deps.get_message") as mock_get_message:
            ...
            response = create_error_response(
                message_key="VALIDATION_ERROR", details={"field": "username"}
            )
