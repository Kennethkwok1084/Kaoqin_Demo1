...
from app.models.task import (
    RepairTask, MonitoringTask, AssistanceTask,
    TaskType, TaskStatus, TaskCategory, TaskPriority, 
    TaskTag, TaskTagType
)
...
class TestCreateRepairTask(TestTaskServiceBasic):
    ...
    @pytest.mark.asyncio
    async def test_create_repair_task_with_auto_assignment(self, service, mock_db, sample_member):
        """测试自动分配报修任务"""
        task_data = {
            "title": "自动分配任务",
            "auto_assign": True
...
class TestCreateMonitoringTask(TestTaskServiceBasic):
    ...
    @pytest.mark.asyncio
    async def test_create_monitoring_task_with_cabinet_count(self, service, mock_db, sample_member):
        """测试带机柜数量的监控任务创建"""
        task_data = {
            "title": "机柜巡检",
            "cabinet_count": 20,
...
class TestCreateAssistanceTask(TestTaskServiceBasic):
    ...
    @pytest.mark.asyncio
    async def test_create_assistance_task_invalid_time_range(self, service, mock_db, sample_member):
        """测试无效时间范围的协助任务"""
        task_data = {
            "title": "无效时间任务",
            "member_id": sample_member.id,
...
class TestUpdateTaskStatus(TestTaskServiceBasic):
    ...
    @pytest.mark.asyncio
    async def test_update_task_status_basic(self, service, mock_db, sample_repair_task, sample_member):
        """测试基础状态更新"""
        # Mock查询结果
        mock_result = MagicMock()
    ...
    @pytest.mark.asyncio
    async def test_update_task_status_to_completed(self, service, mock_db, sample_repair_task, sample_member):
        """测试更新为完成状态"""
        ...
        # Mock工时重算
        with patch.object(service.work_hours_service, 'calculate_task_work_minutes') as mock_calc:
...
class TestAssignTask(TestTaskServiceBasic):
    ...
    @pytest.mark.asyncio
    async def test_assign_task_basic(self, service, mock_db, sample_repair_task, sample_member, admin_member):
        """测试基础任务分配"""
        # Mock查询结果
    ...
    @pytest.mark.asyncio
    async def test_assign_task_member_not_found(self, service, mock_db, sample_repair_task, admin_member):
        """测试分配不存在的成员"""
        task_result = MagicMock()
        task_result.scalar_one_or_none.return_value = sample_repair_task
...
class TestTaskFeedbackAndRating(TestTaskServiceBasic):
    ...
    async def test_add_task_feedback_basic(self, service, mock_db, sample_repair_task):
        ...
        result = await service.add_task_feedback(
            task_id=1,
    ...
    @pytest.mark.asyncio
    async def test_add_task_rating_with_positive_review(self, service, mock_db, sample_repair_task):
        """测试添加正面评分"""
        rating = 5
        feedback = "非常满意的服务"
    ...
    @pytest.mark.asyncio
    async def test_add_task_rating_with_negative_review(self, service, mock_db, sample_repair_task):
        """测试添加负面评分"""
        rating = 2  # 差评
        feedback = "响应速度慢"
        ...
        # Mock工时重算
        with patch.object(service.work_hours_service, 'calculate_task_work_minutes') as mock_calc:
            mock_calc.return_value = {"total_minutes": 0}  # 基础40 - 差评60 = 0 (不能为负)
...
class TestRushTaskManagement(TestTaskServiceBasic):
    ...
    @pytest.mark.asyncio
    async def test_batch_mark_rush_tasks_basic(self, service, mock_db, sample_repair_task):
        """测试批量标记爆单任务"""
        # Mock查询结果
        mock_result = MagicMock()
        ...
        # Mock爆单标记服务
        with patch.object(service.rush_task_service, 'batch_mark_rush_tasks') as mock_rush:
    ...
    @pytest.mark.asyncio
    async def test_get_rush_tasks_list_basic(self, service, mock_db, sample_repair_task):
        """测试获取爆单任务列表"""
        ...
        result = await service.get_rush_tasks_list(
            page=1,
            per_page=10
        )
...
class TestMemberTaskSummary(TestTaskServiceBasic):
    ...
    async def test_get_member_task_summary_basic(self, service, mock_db, sample_member):
        ...
        mock_db.execute.side_effect = [repair_result, monitoring_result, assistance_result]
...
class TestWorkHoursRecalculation(TestTaskServiceBasic):
    ...
    @pytest.mark.asyncio
    async def test_recalculate_task_work_hours_basic(self, service, mock_db, sample_repair_task):
        """测试单个任务工时重算"""
        mock_result = MagicMock()
        mock_result.scalar_one_or_none.return_value = sample_repair_task
        mock_db.execute.return_value = mock_result
    ...
    @pytest.mark.asyncio
    async def test_recalculate_task_work_hours_calculation_error(self, service, mock_db, sample_repair_task):
        """测试工时计算失败的情况"""
        mock_result = MagicMock()
        mock_result.scalar_one_or_none.return_value = sample_repair_task
        mock_db.execute.return_value = mock_result
...
class TestErrorHandling(TestTaskServiceBasic):
    ...
    @pytest.mark.asyncio
    async def test_concurrent_task_modification(self, service, mock_db, sample_repair_task):
        """测试并发任务修改冲突"""
        # 模拟任务在查询后被其他进程修改
        mock_result = MagicMock()
        mock_result.scalar_one_or_none.return_value = sample_repair_task
