...
class TestStatisticsAndReports:
    ...
    async def test_task_performance_analytics(
        ...
        task_distribution_response = await e2e_client.get(
            "/api/v1/statistics/task-type-distribution",
            headers=admin_headers
        )
    ...
    async def test_member_performance_ranking(
        ...
        performance_score_response = await e2e_client.get(
            ...
            params={
                ...
                "include_factors": ["work_hours", "task_completion", "user_satisfaction"]
            },
            headers=admin_headers
        )
    ...
    async def test_geographic_area_analysis(
        ...
        area_efficiency_response = await e2e_client.get(
            "/api/v1/statistics/area-efficiency-comparison",
            headers=admin_headers
        )
    ...
    async def test_word_cloud_analysis(
        ...
        issue_type_wordcloud_response = await e2e_client.get(
            "/api/v1/statistics/issue-type-wordcloud",
            headers=admin_headers
        )
    ...
    async def test_advanced_data_export_functionality(
        ...
        if excel_export_response.status_code == 200:
            ...
            content_disposition = excel_export_response.headers.get("content-disposition", "")
    ...
    async def test_statistical_alerts_and_thresholds(
        ...
        create_alerts_response = await e2e_client.post(
            "/api/v1/statistics/alerts/rules",
            json=alert_rules,
            headers=admin_headers
        )
        ...
        if create_alerts_response.status_code == 200:
            ...
            active_alerts_response = await e2e_client.get(
                "/api/v1/statistics/alerts/active",
                headers=admin_headers
            )
            ...
            if alert_history_response.status_code == 200:
                ...
                if "alert_history" in history_data:
                    ...
                    for alert_record in history:
                        ...
                        assert "resolved_at" in alert_record or alert_record["resolved_at"] is None
        else:
    ...
    async def test_data_visualization_charts(
        ...
        radar_chart_response = await e2e_client.get(
            ...
            params={
                "member_count": 5,
                "metrics": ["work_hours", "task_completion", "response_time", "satisfaction"],
                "format": "svg"
            },
            headers=admin_headers
        )
...
class TestStatisticsPerformance:
    ...
    async def test_statistics_query_performance(
        ...
        if query_times:
            ...
            assert avg_query_time < 2.0, f"Average statistics query time {avg_query_time}s exceeds 2s"
            assert max_query_time < 5.0, f"Max statistics query time {max_query_time}s exceeds 5s"
    ...
    async def test_concurrent_statistics_access(
        ...
        concurrent_requests = [
            ...
            e2e_client.get("/api/v1/statistics/task-type-distribution", headers=admin_headers),
            e2e_client.get("/api/v1/statistics/member-performance", headers=admin_headers),
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                pytest.fail(f"Concurrent statistics request {i} failed with exception: {result}")
            else:
                # 允许200（成功）或404（端点未实现）
                assert result.status_code in [200, 404], f"Concurrent statistics request {i} returned {result.status_code}"