...
class TestUserAuthenticationFlow:
    ...
    async def test_super_admin_exclusive_access(
        ...
        for endpoint in super_admin_endpoints:
            response = await e2e_client.get(endpoint, headers=admin_headers)
            assert response.status_code in [401, 403], f"Regular admin should not access {endpoint}"
        ...
        for endpoint in super_admin_endpoints:
            ...
            assert response.status_code in [200, 404, 405], f"Super admin should access {endpoint}"
    ...
    async def test_session_concurrent_access(
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                ...
                # 允许200或404（资源不存在）
                assert result.status_code in [200, 404], f"Concurrent request {i} failed with status {result.status_code}"
    
    @pytest.mark.asyncio
    async def test_account_activation_flow(
        self,
        e2e_client: AsyncClient,
        e2e_helper
    ):
...
class TestAuthenticationPerformance:
    ...
    async def test_authentication_performance(
        ...
        # 性能断言（可根据实际需求调整阈值）
        assert avg_login_time < 2.0, f"Average login time {avg_login_time}s exceeds 2s threshold"
        assert max_login_time < 5.0, f"Max login time {max_login_time}s exceeds 5s threshold"
