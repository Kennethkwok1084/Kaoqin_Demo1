        ...
        if json_result_file.exists():
            try:
                with open(json_result_file, "r", encoding="utf-8") as f:
                    ...
                    result.update(
                        {
                            "tests_collected": detailed_results.get("summary", {}).get(
                                "total", 0
                            ),
                            "tests_passed": detailed_results.get("summary", {}).get(
                                "passed", 0
                            ),
                            "tests_failed": detailed_results.get("summary", {}).get(
                                "failed", 0
                            ),
                            "tests_skipped": detailed_results.get("summary", {}).get(
                                "skipped", 0
                            ),
                            "tests_error": detailed_results.get("summary", {}).get(
                                "error", 0
                            ),
                        }
                    )
            ...
            for key in [
                ...
                if key in module_result:
                    results["summary"][key.replace("tests_", "")] = (
                        results["summary"].get(key.replace("tests_", ""), 0)
                        + module_result[key]
                    )

            print(
                f"✅ {module}: {module_result['status']} ({module_result['duration']}s)"
            )
...
def main():
    ...
    parser.add_argument(
        "--modules", "-m", nargs="+", help="指定要运行的测试模块 (默认运行全部)"
    )
    parser.add_argument("--verbose", "-v", action="store_true", help="详细输出模式")
    parser.add_argument(
        "--parallel", "-p", action="store_true", help="并行执行测试 (实验性功能)"
    )
    parser.add_argument(
        "--stop-on-failure", "-s", action="store_true", help="遇到失败时停止执行"
    )
    parser.add_argument(
        "--check-only", "-c", action="store_true", help="仅检查环境，不执行测试"
    )
    parser.add_argument(
        "--list-modules", "-l", action="store_true", help="列出所有可用的测试模块"
    )
