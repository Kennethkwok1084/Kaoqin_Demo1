...
class TestSystemSettingsAndPermissions:
    ...
    async def test_work_hours_rules_configuration(
        ...
        if current_rules_response.status_code == 200:
            ...
            assert updated_rules["online_task_base_hours"] == new_rules["online_task_base_hours"]
            assert updated_rules["offline_task_base_hours"] == new_rules["offline_task_base_hours"]
    ...
    async def test_user_role_management_permissions(
        ...
        if super_admin_update_response.status_code == 200:
            ...
            member_response = await e2e_client.get(
                f"/api/v1/members/{student_member_id}",
                headers=super_admin_headers
            )
        ...
        if security_policy_response.status_code == 200:
            ...
            if update_policy_response.status_code == 200:
                ...
                if updated_policy_response.status_code == 200:
                    ...
                    assert updated_policy["password_min_length"] == new_security_policy["password_min_length"]
                    assert updated_policy["max_login_attempts"] == new_security_policy["max_login_attempts"]
...
class TestSystemPermissionsPerformance:
    ...
    async def test_permission_check_performance(
        ...
        assert avg_check_time < 1.0, f"Average permission check time {avg_check_time}s exceeds 1s"
        assert max_check_time < 3.0, f"Max permission check time {max_check_time}s exceeds 3s"
