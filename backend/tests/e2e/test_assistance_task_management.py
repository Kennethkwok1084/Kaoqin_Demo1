...
class TestAssistanceTaskManagement:
    ...
    async def test_student_self_register_assistance_task(
        ...
        if create_response.status_code == 201:
            ...
            task_detail_response = await e2e_client.get(
                f"/api/v1/tasks/assistance/{task_id}",
                headers=student_headers
            )
            ...
            assert task_detail["estimated_duration"] == assistance_data["estimated_duration"]
    ...
    async def test_admin_approval_workflow(
        ...
        pending_tasks_response = await e2e_client.get(
            "/api/v1/tasks/assistance/pending-approval",
        ...
        if pending_tasks_response.status_code == 200:
            ...
            assert task_found, "Created assistance task not found in pending approval list"
    ...
    async def test_admin_rejection_workflow(
        ...
        if rejection_response.status_code == 200:
            ...
            assert rejected_task["rejection_reason"] == rejection_data["rejection_reason"]
        else:
    ...
    async def test_assistance_task_execution_flow(
        ...
        if start_response.status_code == 200:
            ...
            if completion_response.status_code == 200:
                ...
                assert final_task["actual_duration"] == completion_data["actual_duration"]
        else:
    ...
    async def test_assistance_task_work_hours_calculation(
        ...
        if completion_response.status_code == 200:
            ...
            if work_hours_response.status_code == 200:
                ...
                assert work_hours_data["base_hours"] == completion_data["actual_duration"]
        else:
    ...
    async def test_assistance_task_statistics_and_reporting(
        ...
        stats_response = await e2e_client.get(
            "/api/v1/tasks/assistance/statistics",
            headers=admin_headers
        )
        ...
        personal_stats_response = await e2e_client.get(
            "/api/v1/tasks/assistance/my-statistics",
            headers=student_headers
        )
        ...
        leaderboard_response = await e2e_client.get(
            "/api/v1/tasks/assistance/leaderboard",
            headers=admin_headers
        )
    ...
    async def test_assistance_task_notifications(
        ...
        if create_response.status_code == 201:
            ...
            if admin_notifications_response.status_code == 200:
                notifications = admin_notifications_response.json().get("data", {}).get("items", [])
                
                # 查找相关通知
                assistance_notifications = [
                    notif for notif in notifications 
                    if "协助任务" in notif.get("content", "") or "assistance" in notif.get("type", "")
                ]
                
                # 应该有新的协助任务申请通知
                assert len(assistance_notifications) > 0, "No assistance task notification found"
            ...
            if approval_response.status_code == 200:
                ...
                if student_notifications_response.status_code == 200:
                    student_notifications = student_notifications_response.json().get("data", {}).get("items", [])
                    
                    # 查找审批通知
                    approval_notifications = [
                        notif for notif in student_notifications
                        if "审批" in notif.get("content", "") or "approval" in notif.get("type", "")
                    ]
                    
                    assert len(approval_notifications) > 0, "No approval notification found"
        else:
...
class TestAssistanceTaskPerformance:
    ...
    async def test_assistance_task_creation_performance(
        ...
        if creation_times:
            ...
            assert avg_creation_time < 1.5, f"Average assistance creation time {avg_creation_time}s exceeds 1.5s"
            assert max_creation_time < 3.0, f"Max assistance creation time {max_creation_time}s exceeds 3s"
    ...
    async def test_concurrent_assistance_operations(
        ...
        concurrent_tasks = [
            ...
            e2e_client.get("/api/v1/tasks/assistance/pending-approval", headers=admin_headers),
            e2e_client.get("/api/v1/tasks/assistance/my", headers=student_headers),
            e2e_client.get("/api/v1/tasks/assistance/statistics", headers=admin_headers),
            e2e_client.get("/api/v1/tasks/assistance/leaderboard", headers=admin_headers)
        ]
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                pytest.fail(f"Concurrent assistance operation {i} failed with exception: {result}")
            else:
                # 允许200或404（功能未实现）
                assert result.status_code in [200, 404], f"Concurrent assistance operation {i} returned {result.status_code}"