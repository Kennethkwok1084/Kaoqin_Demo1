...
class TestAttendanceDataManagement:
    ...
    async def test_monthly_work_hours_calculation(
        ...
        if calc_response.status_code == 200:
            ...
            if attendance_response.status_code == 200:
                ...
                # 验证考勤记录与计算结果一致
                assert attendance_data["total_work_hours"] == calc_data["total_work_hours"]
    ...
    async def test_attendance_record_crud_operations(
        ...
        if create_response.status_code == 201:
            ...
            # 验证创建的数据
            assert created_record["total_work_hours"] == attendance_data["total_work_hours"]
            ...
            if update_response.status_code == 200:
                ...
                assert updated_record["total_work_hours"] == update_data["total_work_hours"]
        ...
        if export_response.status_code == 200:
            ...
            for format_type in export_formats:
                ...
                if format_response.status_code == 200:
                    # 验证不同格式的内容类型
                    format_content_type = format_response.headers.get("content-type", "")
                    if format_type == "excel":
                        assert "vnd.openxmlformats" in format_content_type or "octet-stream" in format_content_type
                    elif format_type == "csv":
                        assert "text/csv" in format_content_type or "text/plain" in format_content_type
        else:
        ...
        if rollover_response.status_code == 200:
            ...
            if current_attendance_response.status_code == 200:
                ...
                if rollover_info["carried_over_hours"] > 0:
                    ...
                    assert current_data["carried_over_from_previous"] == rollover_info["carried_over_hours"]
        else:
        ...
        if stats_response.status_code == 200:
            ...
            for stat in expected_stats:
                if stat in stats_data:
                    # 验证数据类型和合理性
                    if stat in ["total_members", "total_work_hours", "average_work_hours"]:
    ...
    async def test_attendance_data_validation_and_integrity(
        ...
        if first_create_response.status_code == 201:
            ...
            # 清理测试数据
            await e2e_client.delete(f"/api/v1/attendance/{record_id}", headers=admin_headers)
...
class TestAttendanceDataPerformance:
    ...
    async def test_monthly_calculation_performance(
        ...
        if calculation_times:
            ...
            assert avg_calc_time < 3.0, f"Average monthly calculation time {avg_calc_time}s exceeds 3s"
            assert max_calc_time < 10.0, f"Max monthly calculation time {max_calc_time}s exceeds 10s"
    ...
    async def test_concurrent_attendance_operations(
        ...
        concurrent_tasks = [
            ...
            e2e_client.get("/api/v1/attendance/statistics/overview", headers=admin_headers),
            e2e_client.get("/api/v1/attendance/statistics/monthly-comparison", 
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                pytest.fail(f"Concurrent attendance operation {i} failed with exception: {result}")
            else:
                # 允许200, 404（无数据）或403（权限限制）
                assert result.status_code in [200, 404, 403], f"Concurrent attendance operation {i} returned {result.status_code}"