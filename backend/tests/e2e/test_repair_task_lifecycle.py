...
class TestRepairTaskLifecycle:
    ...
    async def test_repair_task_creation_and_assignment(
        ...
        assignment_data = {
            "assigned_member_id": student_member.id
        }
    ...
    async def test_student_task_processing_flow(
        ...
        start_response = await e2e_client.post(
            f"/api/v1/tasks/repair/{in_progress_task.id}/start",
        ...
        if complete_response.status_code == 200:
            ...
            final_task_response = await e2e_client.get(
                f"/api/v1/tasks/repair/{in_progress_task.id}",
                headers=student_headers
            )
        ...
        if urgent_response.status_code == 201:
            ...
            if rush_response.status_code == 200:
                ...
                task_detail_response = await e2e_client.get(
                    f"/api/v1/tasks/repair/{urgent_task['id']}",
                    headers=admin_headers
                )
    ...
    async def test_task_evaluation_and_feedback(
        ...
        # 为另一个任务添加负面评价（如果有的话）
        other_completed_tasks = [t for t in e2e_sample_repair_tasks 
...
class TestRepairTaskPerformance:
    ...
    async def test_task_creation_performance(
        ...
        assert avg_creation_time < 1.0, f"Average task creation time {avg_creation_time}s exceeds 1s"
        assert max_creation_time < 3.0, f"Max task creation time {max_creation_time}s exceeds 3s"
    ...
    async def test_concurrent_task_operations(
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                ...
                assert result.status_code in [200, 404], f"Concurrent operation {i} returned {result.status_code}"