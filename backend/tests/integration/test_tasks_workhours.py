...
from app.models.task import RepairTask, TaskStatus, TaskPriority, TaskType
...
class TestTasksCRUD:
    ...
    def test_create_task_invalid_data(self, client, auth_headers_admin):
        ...
        response2 = client.post("/api/tasks/repair", 
                               json=task_data2, 
                               headers=auth_headers_admin)
    ...
    def test_get_tasks_with_filters(self, client, auth_headers_admin):
        """测试筛选任务"""
        # 按状态筛选
        response = client.get("/api/tasks/repair?status=pending", 
                             headers=auth_headers_admin)
        ...
        # 按优先级筛选
        response2 = client.get("/api/tasks/repair?priority=high", 
                              headers=auth_headers_admin)
        ...
        # 按执行者筛选
        response3 = client.get(f"/api/tasks/repair?assigned_to={test_member_user.id}", 
                              headers=auth_headers_admin)
    ...
    def test_get_tasks_with_search(self, client, auth_headers_admin):
        """测试搜索任务"""
        response = client.get("/api/tasks/repair?search=网络", 
                             headers=auth_headers_admin)
    ...
    def test_get_task_detail(self, client, auth_headers_admin, db_session, test_member_user):
        """测试获取任务详情"""
        # 先创建一个任务
        ...
        response = client.get(f"/api/tasks/repair/{task.id}", 
                             headers=auth_headers_admin)
    ...
    def test_update_task(self, client, auth_headers_admin, db_session, test_member_user):
        """测试更新任务"""
        # 先创建任务
        from app.models.task import RepairTask
        ...
        response = client.put(f"/api/tasks/repair/{task.id}", 
                             json=update_data, 
                             headers=auth_headers_admin)
    ...
    def test_update_task_status(self, client, auth_headers_member, db_session, test_member_user):
        """测试更新任务状态"""
        # 创建分配给当前用户的任务
        from app.models.task import RepairTask
        ...
        response = client.patch(f"/api/tasks/repair/{task.id}/status", 
                               json=start_data, 
                               headers=auth_headers_member)
        ...
        response2 = client.patch(f"/api/tasks/repair/{task.id}/status", 
                                json=complete_data, 
                                headers=auth_headers_member)
    ...
    def test_assign_task(self, client, auth_headers_admin, db_session, test_member_user, test_group_leader):
        """测试分配任务"""
        # 创建未分配的任务
        from app.models.task import RepairTask
        ...
        response = client.patch(f"/api/tasks/repair/{task.id}/assign", 
                               json=assign_data, 
                               headers=auth_headers_admin)
    ...
    def test_delete_task(self, client, auth_headers_admin, db_session):
        ...
        response = client.delete(f"/api/tasks/repair/{task.id}", 
                                headers=auth_headers_admin)
        ...
        # 验证任务已删除
        get_response = client.get(f"/api/tasks/repair/{task.id}", 
                                 headers=auth_headers_admin)
...
class TestWorkHourCalculation:
    ...
    def test_basic_work_hour_calculation(self, client, auth_headers_admin, test_member_user):
        """测试基础工时计算"""
        # 创建一个已完成的任务
        task_data = {
            "title": "工时计算测试任务",
        ...
        create_response = client.post("/api/tasks/repair", 
                                     json=task_data, 
                                     headers=auth_headers_admin)
        ...
        status_response = client.patch(f"/api/tasks/repair/{task_id}/status", 
                                      json=complete_data, 
                                      headers=auth_headers_admin)
        ...
        response = client.post("/api/tasks/work-hours/calculate", 
                              json=calc_data, 
                              headers=auth_headers_admin)
    ...
    def test_rush_task_bonus(self, client, auth_headers_admin, test_member_user):
        ...
        client.patch(f"/api/tasks/repair/{task_id}/status", 
                    json=complete_data, 
                    headers=auth_headers_admin)
    ...
    def test_negative_review_penalty(self, client, auth_headers_admin, test_member_user):
        """测试差评惩罚"""
        task_data = {
            "title": "差评测试任务",
            "task_type": "repair",
    ...
    def test_online_vs_offline_task_calculation(self, client, auth_headers_admin, test_member_user):
        """测试线上和线下任务工时计算差异"""
        # 创建线上任务
        online_task_data = {
            "title": "线上任务",
        ...
        online_response = client.post("/api/tasks/monitoring", 
                                     json=online_task_data, 
                                     headers=auth_headers_admin)
        ...
        if online_response.status_code == 201:
            ...
            calc_response = client.post("/api/tasks/work-hours/calculate", 
                                       json=calc_data, 
                                       headers=auth_headers_admin)
        ...
        offline_response = client.post("/api/tasks/repair", 
                                      json=offline_task_data, 
                                      headers=auth_headers_admin)
        assert offline_response.status_code == 201
        offline_task_id = offline_response.json()["id"]
        
        complete_data = {"status": "completed", "actual_minutes": 95}
        client.patch(f"/api/tasks/repair/{offline_task_id}/status", 
                    json=complete_data, 
                    headers=auth_headers_admin)
...
class TestTaskAutomation:
    ...
    def test_automatic_overdue_detection(self, client, auth_headers_admin, db_session, test_member_user):
        """测试自动超时检测"""
        # 创建一个已过期的任务
        from app.models.task import RepairTask
        ...
        # 触发自动超时检测
        response = client.post("/api/tasks/automation/detect-overdue", 
                              headers=auth_headers_admin)
    ...
    def test_automatic_penalty_application(self, client, auth_headers_admin, db_session, test_member_user):
        """测试自动惩罚应用"""
        # 创建延迟任务
        from app.models.task import RepairTask
        ...
        # 触发自动惩罚检测
        response = client.post("/api/tasks/automation/apply-penalties", 
                              headers=auth_headers_admin)
...
class TestTaskStatistics:
    ...
    def test_get_task_statistics(self, client, auth_headers_admin, test_data_helper, test_member_user):
        ...
        assert (data["pending_tasks"] + data["in_progress_tasks"] + 
                data["completed_tasks"]) <= data["total_tasks"]
    
    def test_get_member_task_statistics(self, client, auth_headers_member, test_member_user):
        """测试获取个人任务统计"""
        response = client.get(f"/api/members/{test_member_user.id}/tasks/statistics", 
                             headers=auth_headers_member)
