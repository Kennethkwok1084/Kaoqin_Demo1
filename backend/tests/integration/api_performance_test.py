...
class APIPerformanceTester:
    ...
    def run_endpoint_performance_test(
        self, endpoint_config: Dict[str, Any], iterations: int = 10
    ) -> Dict[str, Any]:
        """运行单个端点的性能测试"""
        ...
        for i in range(iterations):
            result = self.measure_response_time(method=method, url=url, params=params)
        ...
        endpoint_result = {
            ...
            "content_length": {
                "average_bytes": (
                    round(statistics.mean(content_lengths), 2) if content_lengths else 0
                )
            },
    ...
    def run_load_test(
        self, endpoint_config: Dict[str, Any], concurrent_requests: int = 5
    ) -> Dict[str, Any]:
        """运行负载测试（模拟并发请求）"""
        endpoint_name = endpoint_config["name"]
        method = endpoint_config["method"]
    ...
    def run_comprehensive_performance_test(self):
        ...
        test_endpoints = [
            # 健康检查API（无需认证）
            {"name": "Root Health Check", "method": "GET", "url": "/", "params": {}},
            {
        ...
        health_check_endpoints = [
            ep for ep in test_endpoints if "health" in ep["name"].lower()
        ]
    ...
    def generate_performance_report(self) -> str:
        ...
        for name, result in self.results["endpoint_results"].items():
            ...
            report.append(
                f"    * Average: {result['response_times']['average_ms']:.2f}ms"
            )
