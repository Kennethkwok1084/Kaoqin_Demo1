...
class TestMembersCRUD:
    ...
    def test_create_member_invalid_data(self, client, auth_headers_admin):
        ...
        response2 = client.post("/api/members", 
                               json=member_data2, 
                               headers=auth_headers_admin)
    ...
    def test_get_member_detail_as_admin(self, client, auth_headers_admin, test_member_user):
        """测试管理员获取成员详情"""
        response = client.get(f"/api/members/{test_member_user.id}", 
                             headers=auth_headers_admin)
    ...
    def test_get_member_detail_as_self(self, client, auth_headers_member, test_member_user):
        """测试获取自己的详情"""
        response = client.get(f"/api/members/{test_member_user.id}", 
                             headers=auth_headers_member)
    ...
    def test_get_member_detail_as_other(self, client, auth_headers_member, test_admin_user):
        """测试获取他人详情"""
        response = client.get(f"/api/members/{test_admin_user.id}", 
                             headers=auth_headers_member)
    ...
    def test_update_member_as_admin(self, client, auth_headers_admin, test_member_user):
        ...
        response = client.put(f"/api/members/{test_member_user.id}", 
                             json=update_data, 
                             headers=auth_headers_admin)
    ...
    def test_update_member_role_as_admin(self, client, auth_headers_admin, test_member_user):
        """测试管理员更新成员角色"""
        role_data = {"role": "group_leader"}
        
        response = client.patch(f"/api/members/{test_member_user.id}/role", 
                               json=role_data, 
                               headers=auth_headers_admin)
    ...
    def test_update_member_status_as_admin(self, client, auth_headers_admin, test_member_user):
        """测试管理员更新成员状态"""
        status_data = {
            "is_active": False,
            "reason": "违反规定"
        ...
        response = client.patch(f"/api/members/{test_member_user.id}/status", 
                               json=status_data, 
                               headers=auth_headers_admin)
    ...
    def test_update_member_as_non_admin(self, client, auth_headers_member, test_admin_user):
        """测试非管理员更新成员"""
        update_data = {"name": "不应成功"}
        
        response = client.put(f"/api/members/{test_admin_user.id}", 
                             json=update_data, 
                             headers=auth_headers_member)
    ...
    def test_delete_member_as_admin(self, client, auth_headers_admin, db_session):
        ...
        response = client.delete(f"/api/members/{to_delete.id}", 
                                headers=auth_headers_admin)
        ...
        # 验证成员确实被删除
        get_response = client.get(f"/api/members/{to_delete.id}", 
                                 headers=auth_headers_admin)
        assert get_response.status_code == 404
    
    def test_delete_member_as_non_admin(self, client, auth_headers_member, test_admin_user):
        """测试非管理员删除成员"""
        response = client.delete(f"/api/members/{test_admin_user.id}", 
                                headers=auth_headers_member)
...
class TestMembersBulkOperations:
    ...
    def test_bulk_activate_members(self, client, auth_headers_admin, bulk_test_members):
        ...
        response = client.post("/api/members/bulk", 
                              json=bulk_data, 
                              headers=auth_headers_admin)
    ...
    def test_bulk_deactivate_members(self, client, auth_headers_admin, bulk_test_members):
        """测试批量停用成员"""
    ...
    def test_bulk_operation_as_non_admin(self, client, auth_headers_member, bulk_test_members):
        """测试非管理员批量操作"""
        member_ids = [member.id for member in bulk_test_members[:2]]
...
class TestMembersStatistics:
    ...
    def test_get_members_statistics(self, client, auth_headers_admin):
        ...
        assert data["active_members"] + data["inactive_members"] == data["total_members"]
        assert data["admin_count"] + data["group_leader_count"] + data["member_count"] == data["total_members"]
