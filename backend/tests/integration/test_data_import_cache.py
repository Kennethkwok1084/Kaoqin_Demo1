...
class TestExcelDataImport:
    ...
    def test_upload_excel_file(self, client, auth_headers_admin):
        ...
        excel_data = {
            ...
            "维修类型": ["线下维修", "线上维修", "线下维修"],
            "紧急程度": ["高", "中", "低"],
            "报告时间": ["2025-01-29 09:00:00", "2025-01-29 10:30:00", "2025-01-29 14:15:00"]
        }
        ...
        # 模拟文件上传
        files = {
            "file": ("repair_data.xlsx", excel_buffer.getvalue(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
        }
    ...
    def test_preview_import_data(self, client, auth_headers_admin):
        ...
        response = client.get(f"/api/data-import/preview/{file_id}", 
                             headers=auth_headers_admin)
    ...
    def test_validate_import_data(self, client, auth_headers_admin):
        ...
        response = client.post("/api/data-import/validate", 
        ...
        if response.status_code == 200:
            ...
            # 验证数据合理性
            assert summary["valid_count"] + summary["invalid_count"] == summary["total_rows"]
        else:
            # 如果功能未实现
    ...
    def test_execute_data_import(self, client, auth_headers_admin):
        ...
        response = client.post("/api/data-import/execute", 
                              json=import_data, 
                              headers=auth_headers_admin)
        ...
        if response.status_code == 200:
            ...
            # 验证数据合理性
            assert summary["successful_count"] + summary["failed_count"] == summary["total_processed"]
    ...
    def test_get_import_detail(self, client, auth_headers_admin):
        ...
        response = client.get(f"/api/data-import/history/{import_id}", 
                             headers=auth_headers_admin)
...
class TestDataMatching:
    ...
    def test_ab_table_matching_algorithm(self, client, auth_headers_admin, db_session, test_member_user):
        """测试A/B表匹配算法"""
        # 创建A表数据（任务表）
        ...
        response = client.post("/api/data-matching/ab-tables", 
                              json=matching_data, 
                              headers=auth_headers_admin)
    ...
    def test_fuzzy_matching_with_typos(self, client, auth_headers_admin):
        ...
        response = client.post("/api/data-matching/fuzzy-match", 
                              json=fuzzy_data, 
                              headers=auth_headers_admin)
    ...
    def test_duplicate_detection(self, client, auth_headers_admin):
        ...
        duplicate_data = {
            "data_records": [
                ...
                {"name": "张三", "contact": "13812345678", "location": "A101"},  # 完全重复
                {"name": "张三", "contact": "13812345678", "location": "A102"},  # 部分重复
                {"name": "李四", "contact": "13987654321", "location": "B203"},
                {"name": "李四", "contact": "13987654322", "location": "B203"}   # 相似但不重复
            ],
            "detection_rules": {
        ...
        response = client.post("/api/data-matching/detect-duplicates", 
                              json=duplicate_data, 
                              headers=auth_headers_admin)
...
class TestBackgroundJobs:
    ...
    def test_schedule_data_processing_job(self, mock_send_task, client, auth_headers_admin):
        """测试调度数据处理任务"""
        # 模拟Celery任务调度
        ...
        response = client.post("/api/jobs/schedule", 
                              json=job_data, 
                              headers=auth_headers_admin)
    ...
    def test_get_job_status(self, client, auth_headers_admin):
        ...
        if response.status_code == 200:
            ...
            # 验证状态值
            assert data["status"] in ["PENDING", "STARTED", "SUCCESS", "FAILURE", "RETRY", "REVOKED"]
...
class TestDataImportIntegration:
    ...
    def test_full_import_workflow(self, client, auth_headers_admin, db_session, test_member_user):
        """测试完整的数据导入工作流程"""
        # 1. 上传Excel文件
        excel_data = {
            "报告人姓名": ["测试用户", "另一用户"],
        ...
        files = {
            "file": ("test_data.xlsx", excel_buffer.getvalue(), 
                    "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
        }
        
        upload_response = client.post("/api/data-import/upload", 
                                    files=files, 
                                    headers=auth_headers_admin)
        ...
        # 2. 预览和验证数据
        preview_response = client.get(f"/api/data-import/preview/{file_id}", 
                                    headers=auth_headers_admin)
        ...
        if preview_response.status_code == 200:
            ...
            validation_response = client.post("/api/data-import/validate", 
                                            json=validation_data, 
                                            headers=auth_headers_admin)
            ...
            if validation_response.status_code == 200:
                ...
                import_response = client.post("/api/data-import/execute", 
                                            json=import_data, 
                                            headers=auth_headers_admin)
                ...
                if import_response.status_code == 200:
                    ...
                    # 5. 验证任务是否创建成功
                    tasks_response = client.get("/api/tasks/repair", 
                                               headers=auth_headers_admin)
                    
                    if tasks_response.status_code == 200:
                        tasks_data = tasks_response.json()
                        created_tasks = [task for task in tasks_data["items"] 
                                       if task.get("reporter_name") in ["测试用户", "另一用户"]]
                        
                        assert len(created_tasks) == import_result["import_summary"]["successful_count"]
                        
                        # 验证任务数据正确性
                        for task in created_tasks:
                            assert task["reporter_name"] in ["测试用户", "另一用户"]
                            assert task["reporter_contact"] in ["13812345678", "13987654321"]
                            assert task["status"] == "pending"
                            assert "task_number" in task
        
        # 6. 检查导入历史
        history_response = client.get("/api/data-import/history", 
                                    headers=auth_headers_admin)
        ...
        if history_response.status_code == 200:
            ...
            # 验证历史记录中包含本次导入
            recent_imports = [item for item in history_data["items"] 
