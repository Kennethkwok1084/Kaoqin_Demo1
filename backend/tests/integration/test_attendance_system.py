...
class TestAttendanceCheckinCheckout:
    ...
    def test_checkin_success(self, client, auth_headers_member):
        """测试成功签到"""
        checkin_data = {
            "checkin_time": datetime.now().replace(hour=9, minute=0, second=0, microsecond=0).isoformat(),
            "location": "办公室",
            "notes": "正常签到"
        }
        
        response = client.post("/api/attendance/checkin", 
                              json=checkin_data, 
    ...
    def test_checkin_late(self, client, auth_headers_member):
        """测试迟到签到"""
        # 10点签到（假设9点上班）
        late_checkin_time = datetime.now().replace(hour=10, minute=30, second=0, microsecond=0)
    ...
    def test_checkin_duplicate_same_day(self, client, auth_headers_member):
        ...
        # 第一次签到
        response1 = client.post("/api/attendance/checkin", 
                               json=checkin_data, 
                               headers=auth_headers_member)
        assert response1.status_code == 200
        
        # 第二次签到（同一天）
        response2 = client.post("/api/attendance/checkin", 
                               json=checkin_data, 
                               headers=auth_headers_member)
    ...
    def test_checkout_success(self, client, auth_headers_member):
        ...
        checkin_response = client.post("/api/attendance/checkin", 
                                      json=checkin_data, 
                                      headers=auth_headers_member)
        ...
        response = client.post("/api/attendance/checkout", 
                              json=checkout_data, 
                              headers=auth_headers_member)
        ...
        # 验证签退响应
        assert data["success"] is True
        assert "签退成功" in data["message"] or "checked out successfully" in data["message"]
    ...
    def test_checkout_early(self, client, auth_headers_member):
        ...
        client.post("/api/attendance/checkin", json=checkin_data, headers=auth_headers_member)
...
class TestAttendanceRecords:
    ...
    def test_get_attendance_record_detail(self, client, auth_headers_member, db_session, test_member_user):
        """测试获取考勤记录详情"""
        # 先创建一个考勤记录
        ...
        response = client.get(f"/api/attendance/records/{record.id}", 
                             headers=auth_headers_member)
    ...
    def test_update_attendance_record(self, client, auth_headers_admin, db_session, test_member_user):
        """测试更新考勤记录（管理员权限）"""
        # 创建考勤记录
        from app.models.attendance import AttendanceRecord
        ...
        response = client.put(f"/api/attendance/records/{record.id}", 
                             json=update_data, 
    ...
    def test_delete_attendance_record(self, client, auth_headers_admin, db_session, test_member_user):
        """测试删除考勤记录"""
        # 创建待删除记录
        from app.models.attendance import AttendanceRecord
        ...
        response = client.delete(f"/api/attendance/records/{record.id}", 
                                headers=auth_headers_admin)
        ...
        # 验证记录已删除
        get_response = client.get(f"/api/attendance/records/{record.id}", 
                                 headers=auth_headers_admin)
...
class TestAttendanceExceptions:
    ...
    def test_submit_attendance_exception(self, client, auth_headers_member):
        ...
        response = client.post("/api/attendance/exceptions", 
                              json=exception_data, 
                              headers=auth_headers_member)
    ...
    def test_get_attendance_exceptions(self, client, auth_headers_member, db_session, test_member_user):
        """测试获取考勤异常列表"""
        # 创建一些异常记录
    ...
    def test_get_exception_detail(self, client, auth_headers_member, db_session, test_member_user):
        """测试获取异常详情"""
        from app.models.attendance import AttendanceException
        ...
        response = client.get(f"/api/attendance/exceptions/{exception.id}", 
                             headers=auth_headers_member)
    ...
    def test_approve_exception_as_admin(self, client, auth_headers_admin, db_session, test_member_user):
        """测试管理员批准异常申请"""
        from app.models.attendance import AttendanceException
        ...
        approve_data = {
            "status": "approved",
            "reviewer_comments": "情况属实，批准请假"
        }
        
        response = client.patch(f"/api/attendance/exceptions/{exception.id}", 
                               json=approve_data, 
                               headers=auth_headers_admin)
    ...
    def test_reject_exception_as_admin(self, client, auth_headers_admin, db_session, test_member_user):
        """测试管理员拒绝异常申请"""
        from app.models.attendance import AttendanceException
    ...
    def test_process_exception_as_non_admin(self, client, auth_headers_member, db_session, test_admin_user):
        """测试非管理员处理异常申请"""
        from app.models.attendance import AttendanceException
...
class TestAttendanceStatistics:
    ...
    def test_export_attendance_data(self, client, auth_headers_admin):
        ...
        response = client.post("/api/attendance/export", 
                              json=export_data, 
                              headers=auth_headers_admin)
        ...
        if response.status_code == 200:
            ...
            # 验证文件信息
            assert data["filename"].endswith(".xlsx") or data["filename"].endswith(".csv")
            assert data["total_records"] >= 0
        else:
            # 如果功能未实现
...
class TestAttendanceBulkOperations:
    ...
    def test_bulk_import_attendance(self, client, auth_headers_admin, test_member_user, test_group_leader):
        """测试批量导入考勤记录"""
        import_data = {
            "records": [
                {
                ...
                },
                {
                    ...
                    "attendance_date": date.today().isoformat(),
                    "checkin_time": datetime.now().replace(hour=8, minute=30).isoformat(),
                    "checkout_time": datetime.now().replace(hour=17, minute=30).isoformat(),
                    "location": "办公室",
                    "notes": "批量导入记录2"
                }
            ]
        ...
        response = client.post("/api/attendance/bulk-import", 
                              json=import_data, 
                              headers=auth_headers_admin)
        ...
        if response.status_code == 200:
            ...
            assert data["successful_records"] + data["failed_records"] == data["total_records"]
            assert "errors" in data
        else:
            # 如果功能未实现
            assert response.status_code in [404, 405, 501]
    
    def test_bulk_update_attendance_status(self, client, auth_headers_admin, db_session, test_member_user):
        """测试批量更新考勤状态"""
        # 创建一些考勤记录
        from app.models.attendance import AttendanceRecord
        ...
        response = client.patch("/api/attendance/bulk-update", 
                               json=update_data, 
                               headers=auth_headers_admin)
...
class TestAttendanceIntegration:
    ...
    def test_full_attendance_workflow(self, client, auth_headers_member, auth_headers_admin):
        """测试完整考勤流程"""
        # 1. 签到
        checkin_response = client.post("/api/attendance/checkin", 
                                      json={"location": "办公室"}, 
        ...
        # 2. 签退
        checkout_response = client.post("/api/attendance/checkout", 
                                       json={"location": "办公室"}, 
                                       headers=auth_headers_member)
        ...
        # 4. 提交异常申请
        exception_response = client.post("/api/attendance/exceptions", 
                                        json={
                                            "exception_type": "备注",
                                            "exception_date": date.today().isoformat(),
                                            "reason": "补充说明今日工作内容"
        ...
        # 5. 管理员处理异常
        approve_response = client.patch(f"/api/attendance/exceptions/{exception_id}", 
                                       json={
                                           "status": "approved",
                                           "reviewer_comments": "已确认"
                                       }, 
                                       headers=auth_headers_admin)
        ...
        # 6. 查看月度汇总
        today = date.today()
        summary_response = client.get(f"/api/attendance/summary/{today.year}/{today.month}", 
                                     headers=auth_headers_member)
