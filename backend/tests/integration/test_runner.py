...
class IntegrationTestRunner:
    ...
    def run_test_suite(
        ...
        if generate_html_report:
            html_report_path = (
                Path(self.test_directory).parent
                / "reports"
        ...
        report = {
            ...
            "test_statistics": {
                **stats,
                ...
                "failure_rate": round(
                    (stats["failed"] / total_tests * 100) if total_tests > 0 else 0, 2
                ),
            },
    ...
    def _analyze_results_by_module(
        self, test_results: Dict[str, Any]
    ) -> Dict[str, Any]:
        """按测试模块分析结果"""
        modules = {
        ...
        for module_name, module_stats in modules.items():
            ...
            if total > 0:
                module_stats["success_rate"] = round(
                    module_stats["passed"] / total * 100, 2
                )
            else:
    ...
    def _generate_markdown_report(
        ...
        with open(output_file, "w", encoding="utf-8") as f:
            ...
            for module_name, module_stats in report["test_results_by_module"].items():
                f.write(
                    f"| {module_name} | {module_stats['total']} | "
...
def main():
    ...
    try:
        results = runner.run_test_suite(
            generate_html_report=True, generate_json_report=True, coverage_report=True
        )
