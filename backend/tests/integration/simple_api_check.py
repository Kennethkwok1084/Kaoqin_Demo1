...
def check_api_imports():
    ...
    for module_name, description in import_tests:
        ...
        try:
            ...
            results["imports"][module_name] = {"status": "OK", "description": description}
            results["success"] += 1
            print(f"[OK] {description}: {module_name}")
        except ImportError as e:
            results["imports"][module_name] = {"status": "FAIL", "description": description, "error": str(e)}
            print(f"[FAIL] {description}: {module_name} - {e}")
        except Exception as e:
            results["imports"][module_name] = {"status": "ERROR", "description": description, "error": str(e)}
...
def main():
    ...
    print(f"Import success rate: {import_success_rate:.1f}% ({import_results['success']}/{import_results['total']})")
    
    if "error" not in route_results:
        route_success_rate = (route_results["new_paths_found"] / route_results["new_paths_expected"]) * 100
        print(f"New API routes found: {route_success_rate:.1f}% ({route_results['new_paths_found']}/{route_results['new_paths_expected']})")
    ...
    if "error" not in response_results:
        ...
        response_success_rate = (successful_responses / total_responses) * 100 if total_responses > 0 else 0
        print(f"API response success rate: {response_success_rate:.1f}% ({successful_responses}/{total_responses})")
