...
class TestAuthenticationFlow:
    ...
    def test_protected_endpoint_with_invalid_token(self, client):
        ...
        assert response.status_code == 401
        data = response.json()
        assert "Could not validate credentials" in data["detail"] or "认证失败" in data["detail"]
...
class TestPasswordChange:
    ...
    def test_change_password_success(self, client, auth_headers_member):
        ...
        data = response.json()
        assert data["success"] is True
        assert "密码修改成功" in data["message"] or "Password changed successfully" in data["message"]
    ...
    def test_change_password_wrong_current(self, client, auth_headers_member):
        ...
        data = response.json()
        assert data["success"] is False
        assert "当前密码错误" in data["message"] or "Current password is incorrect" in data["message"]
