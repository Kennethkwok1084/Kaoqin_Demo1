...
class StatisticsService:
    ...
    def _generate_cache_key(self, method_name: str, **kwargs) -> str:
        ...
        return f"stats:{method_name}:{param_string}" if param_string else f"stats:{method_name}"
    ...
    async def get_overview_statistics(
        ...
        try:
            ...
            if use_cache and self.cache_enabled:
                cached_data = await cache.get_stats_cache("overview", 
                    date_from=date_from.isoformat(),
                    date_to=date_to.isoformat()
                )
                if cached_data:
            ...
            member_stats_task = asyncio.create_task(self._get_member_statistics_cached())
            task_stats_task = asyncio.create_task(self._get_task_statistics_cached(date_from, date_to))
            work_hour_stats_task = asyncio.create_task(self._get_work_hour_statistics_cached(date_from, date_to))
            performance_stats_task = asyncio.create_task(self._get_performance_statistics_cached(date_from, date_to))
            attendance_stats_task = asyncio.create_task(self._get_attendance_statistics_cached(date_from, date_to))
            
            # 等待所有任务完成
            member_stats, task_stats, work_hour_stats, performance_stats, attendance_stats = await asyncio.gather(
                member_stats_task,
                task_stats_task, 
            ...
            # 存入缓存（10分钟过期）
            if use_cache and self.cache_enabled:
                await cache.set_stats_cache("overview", result, ttl=600,
                    date_from=date_from.isoformat(),
                    date_to=date_to.isoformat()
                )
    ...
    async def _get_task_statistics_cached(self, date_from: datetime, date_to: datetime) -> Dict[str, Any]:
        """获取任务统计（缓存版本）"""
        # 尝试从缓存获取
        cached_data = await cache.get_stats_cache("task",
            date_from=date_from.isoformat(),
            date_to=date_to.isoformat()
        )
        if cached_data:
        ...
        repair_query = select(
            ...
            func.count(case((RepairTask.status == TaskStatus.COMPLETED, 1))).label('completed_count'),
            func.count(case((RepairTask.status == TaskStatus.PENDING, 1))).label('pending_count'),
            func.count(case((RepairTask.status == TaskStatus.IN_PROGRESS, 1))).label('in_progress_count'),
            func.count(case((RepairTask.task_type == TaskType.ONLINE, 1))).label('online_count'),
            func.count(case((RepairTask.task_type == TaskType.OFFLINE, 1))).label('offline_count'),
        ...
        data = {
            "repair_tasks": {
                ...
                "completion_rate": round((repair_stats.completed_count or 0) / max(repair_stats.total_count or 1, 1) * 100, 2),
            ...
            "total_count": (repair_stats.total_count or 0) + (monitoring_stats.total_count or 0) + (assistance_stats.total_count or 0),
            "completed_count": repair_stats.completed_count or 0
        }
        
        # 存入缓存（5分钟过期）
        await cache.set_stats_cache("task", data, ttl=300,
            date_from=date_from.isoformat(),
            date_to=date_to.isoformat()
        )
        return data
    
    async def _get_work_hour_statistics_cached(self, date_from: datetime, date_to: datetime) -> Dict[str, Any]:
        """获取工时统计（缓存版本）"""
        # 尝试从缓存获取
        cached_data = await cache.get_stats_cache("work_hour",
            date_from=date_from.isoformat(),
            date_to=date_to.isoformat()
        )
        if cached_data:
        ...
        work_hour_query = select(
            ...
            RepairTask.__table__.outerjoin(MonitoringTask.__table__).outerjoin(AssistanceTask.__table__)
        ).where(
            or_(
                and_(RepairTask.report_time >= date_from, RepairTask.report_time <= date_to),
                and_(MonitoringTask.start_time >= date_from, MonitoringTask.start_time <= date_to),
                and_(AssistanceTask.start_time >= date_from, AssistanceTask.start_time <= date_to)
            )
        )
        ...
        # 存入缓存（5分钟过期）
        await cache.set_stats_cache("work_hour", data, ttl=300,
            date_from=date_from.isoformat(),
            date_to=date_to.isoformat()
        )
        return data
    
    async def _get_performance_statistics_cached(self, date_from: datetime, date_to: datetime) -> Dict[str, Any]:
        """获取绩效统计（缓存版本）"""
        # 尝试从缓存获取
        cached_data = await cache.get_stats_cache("performance",
            date_from=date_from.isoformat(),
            date_to=date_to.isoformat()
        )
        if cached_data:
        ...
        data = {
            ...
            "good_rating_rate": round((stats.good_rating_count or 0) / max(rated_count, 1) * 100, 2),
            "poor_rating_rate": round((stats.poor_rating_count or 0) / max(rated_count, 1) * 100, 2)
        }
        
        # 存入缓存（5分钟过期）
        await cache.set_stats_cache("performance", data, ttl=300,
            date_from=date_from.isoformat(),
            date_to=date_to.isoformat()
        )
        return data
    
    async def _get_attendance_statistics_cached(self, date_from: datetime, date_to: datetime) -> Dict[str, Any]:
        """获取考勤统计（缓存版本）"""
        # 尝试从缓存获取
        cached_data = await cache.get_stats_cache("attendance",
            date_from=date_from.isoformat(),
            date_to=date_to.isoformat()
        )
        if cached_data:
        ...
        try:
            ...
            attendance_query = select(
                ...
                func.count(case((AttendanceRecord.checkin_time.isnot(None), 1))).label('checkin_count'),
                func.count(case((AttendanceRecord.checkout_time.isnot(None), 1))).label('checkout_count'),
                func.count(case((AttendanceRecord.is_late_checkin == True, 1))).label('late_count'),
                func.count(case((AttendanceRecord.is_early_checkout == True, 1))).label('early_count'),
            ...
            data = {
                ...
                "late_rate": round((stats.late_count or 0) / max(checkin_count, 1) * 100, 2),
                "early_checkout_rate": round((stats.early_count or 0) / max(stats.checkout_count or 1, 1) * 100, 2),
        ...
        # 存入缓存（5分钟过期）
        await cache.set_stats_cache("attendance", data, ttl=300,
            date_from=date_from.isoformat(),
            date_to=date_to.isoformat()
        )
        return data
    ...
    async def get_member_performance_report(
        ...
        try:
            ...
            # 任务完成情况
            task_stats = await self._get_member_task_statistics(member_id, start_date, end_date)
            
            # 工时统计
            work_hour_stats = await self._get_member_work_hour_statistics(member_id, start_date, end_date)
            
            # 质量分析
            quality_stats = await self._get_member_quality_statistics(member_id, start_date, end_date)
            
            # 排名分析
            ranking_stats = await self._get_member_ranking(member_id, start_date, end_date)
    ...
    async def get_team_comparison_report(
        ...
        try:
            ...
            for member in members:
                stats = await self._get_member_comparison_stats(member.id, date_from, date_to)
                stats["member"] = {
                    "id": member.id,
                    "name": member.name,
                    "student_id": member.student_id,
            ...
            # 按工时排序
            member_stats.sort(key=lambda x: x["work_hours"]["total_hours"], reverse=True)
    ...
    async def get_monthly_trends(
        ...
        try:
            ...
            for i in range(months):
                ...
                # 获取月度统计
                monthly_stats = await self._get_monthly_statistics(start_date, end_date, group_id)
    ...
    async def export_statistics_data(
        ...
        try:
            if export_type == "summary":
                ...
                return await self._export_detailed_data(date_from, date_to, member_ids, group_id)
    ...
    async def _get_task_statistics(self, date_from: datetime, date_to: datetime) -> Dict[str, Any]:
        """获取任务统计"""
    ...
    async def _get_work_hour_statistics(self, date_from: datetime, date_to: datetime) -> Dict[str, Any]:
        """获取工时统计"""
        # 维修任务工时
    ...
    async def _get_performance_statistics(self, date_from: datetime, date_to: datetime) -> Dict[str, Any]:
        """获取绩效统计"""
        # 平均评分
    ...
    async def _get_member_task_statistics(self, member_id: int, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        """获取成员任务统计"""
        base_query = select(RepairTask).where(
            and_(
    ...
    async def _get_member_work_hour_statistics(self, member_id: int, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        """获取成员工时统计"""
        # 维修任务工时
        repair_query = select(func.sum(RepairTask.work_minutes)).where(
            and_(
    ...
    async def _get_member_quality_statistics(self, member_id: int, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        """获取成员质量统计"""
        ...
        positive_rate = round((positive_reviews / len(rated_tasks) * 100), 2) if rated_tasks else 0
    ...
    async def _get_member_ranking(self, member_id: int, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        """获取成员排名"""
        # 获取所有活跃成员的工时
        all_members_query = select(
            Member.id,
            Member.name,
            func.coalesce(func.sum(RepairTask.work_minutes), 0).label('total_minutes')
        ).outerjoin(
            RepairTask,
            and_(
        ...
        ).group_by(Member.id, Member.name).order_by(desc('total_minutes'))
        ...
        return {
            ...
            "total_members": total_members,
            "percentile": round((1 - (member_rank - 1) / total_members) * 100, 1) if total_members > 0 else 0
        }
    
    async def _get_member_trend_analysis(self, member_id: int, year: int, month: int) -> Dict[str, Any]:
        """获取成员趋势分析"""
        # 获取最近3个月的数据进行对比
        trends = []
        ...
        for i in range(3):
            ...
            # 获取月度数据
            monthly_data = await self._get_member_work_hour_statistics(member_id, start_date, end_date)
        ...
        if len(trends) >= 2:
            ...
            if previous_hours > 0:
                change_rate = round(((current_hours - previous_hours) / previous_hours * 100), 2)
            else:
        ...
        return {
            ...
            "trend_direction": "up" if change_rate > 0 else "down" if change_rate < 0 else "stable"
        }
    
    async def _get_member_comparison_stats(self, member_id: int, date_from: datetime, date_to: datetime) -> Dict[str, Any]:
        """获取成员对比统计"""
        task_stats = await self._get_member_task_statistics(member_id, date_from, date_to)
        work_hour_stats = await self._get_member_work_hour_statistics(member_id, date_from, date_to)
        quality_stats = await self._get_member_quality_statistics(member_id, date_from, date_to)
    ...
    async def _calculate_team_summary(self, member_stats: List[Dict[str, Any]]) -> Dict[str, Any]:
        """计算团队汇总"""
        if not member_stats:
            return {}
        ...
        total_completed = sum(stats["tasks"]["completed_tasks"] for stats in member_stats)
        
        avg_rating_sum = sum(stats["quality"]["avg_rating"] for stats in member_stats if stats["quality"]["avg_rating"] > 0)
        rated_members = len([stats for stats in member_stats if stats["quality"]["avg_rating"] > 0])
        team_avg_rating = round(avg_rating_sum / rated_members, 2) if rated_members > 0 else 0
        ...
        return {
            ...
            "team_completion_rate": round((total_completed / total_tasks * 100), 2) if total_tasks > 0 else 0,
            "team_avg_rating": team_avg_rating
        }
    
    async def _get_monthly_statistics(self, start_date: datetime, end_date: datetime, group_id: Optional[int] = None) -> Dict[str, Any]:
        """获取月度统计"""
        # 任务统计
    ...
    def _analyze_trends(self, trends: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # 简单的趋势分析
        work_hour_trend = "increasing" if work_hours[-1] > work_hours[0] else "decreasing"
    ...
    async def _export_summary_data(self, date_from: datetime, date_to: datetime, group_id: Optional[int] = None) -> Dict[str, Any]:
        """导出汇总数据"""
    ...
    async def _export_detailed_data(self, date_from: datetime, date_to: datetime, member_ids: Optional[List[int]] = None, group_id: Optional[int] = None) -> Dict[str, Any]:
        """导出详细数据"""
        # 获取成员列表
        member_query = select(Member).where(Member.is_active == True)
        if member_ids:
        ...
        detailed_data = []
        for member in members:
            member_data = await self._get_member_comparison_stats(member.id, date_from, date_to)
            member_data["member"] = {
                "id": member.id,
                "name": member.name,
                "student_id": member.student_id,
    ...
    async def _export_comparison_data(self, date_from: datetime, date_to: datetime, group_id: Optional[int] = None) -> Dict[str, Any]:
        """导出对比数据"""
        comparison_report = await self.get_team_comparison_report(group_id, date_from, date_to)
