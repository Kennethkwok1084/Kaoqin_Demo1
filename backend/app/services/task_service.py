...
from app.models.task import (
    ...
    TaskType, TaskPriority, TaskCategory, TaskTagType, task_tag_association
)
...
class TaskService:
    ...
    async def create_monitoring_task(
        ...
        try:
            ...
            logger.info(f"Monitoring task created: {task.id}, work_minutes: {work_minutes}")
            return task
    ...
    async def create_assistance_task(
        ...
        try:
            ...
            logger.info(f"Assistance task created: {task.id}, work_minutes: {work_minutes}")
            return task
    ...
    async def get_rush_tasks_list(
        ...
        try:
            ...
            query = query.order_by(desc(RepairTask.report_time)).limit(page_size).offset(offset)
            ...
            for task in tasks:
                task_data = {
                    ...
                    "rush_info": task.get_rush_order_info() if hasattr(task, 'get_rush_order_info') else None
                }
    ...
    async def get_rush_tasks_statistics(
        ...
        try:
            ...
            for status in TaskStatus:
                status_stats[status.value] = len([t for t in rush_tasks if t.status == status])
            
            # 按任务类型统计
            type_stats = {
                "online": len([t for t in rush_tasks if t.task_type == TaskType.ONLINE]),
                "offline": len([t for t in rush_tasks if t.task_type == TaskType.OFFLINE])
            }
            ...
            return {
                ...
                "summary": {
                    ...
                    "average_task_hours": round(total_work_hours / total_tasks, 2) if total_tasks > 0 else 0
                },
    ...
    async def remove_rush_task_marking(
        ...
        try:
            ...
            logger.info(f"Removed rush marking from {removed_count} tasks by user {remover_id}")
...
class AssistanceTaskService:
    ...
    async def import_maintenance_orders(
        ...
        """
        try:
            import_batch_id = f"import_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}_{importer_id}"
            ...
            if auto_match and len(maintenance_data) > 1:
                maintenance_data = await self._perform_ab_table_matching(maintenance_data)
                matched_count = len([item for item in maintenance_data if item.get("_matched", False)])
            ...
            for data in maintenance_data:
                try:
                    ...
                    if existing_task:
                        # 更新现有任务
                        await self._update_existing_maintenance_task(existing_task, data)
                        updated_count += 1
                    else:
                        # 创建新任务
                        await self._create_maintenance_task(data, importer_id, import_batch_id)
                        created_count += 1
            ...
            if len(indices) > 1:
                ...
                if primary_record:
                    for secondary in secondary_records:
                        primary_record = self._merge_maintenance_records(primary_record, secondary)
    ...
    def _merge_maintenance_records(
        ...
        fields_to_merge = {
            "work_order_id": secondary.get("work_order_id") or primary.get("work_order_id"),
            "company": secondary.get("company") or primary.get("company"),
            "location": secondary.get("location") or primary.get("location"),
            "contact_person": secondary.get("contact_person") or primary.get("contact_person"),
            "contact_phone": secondary.get("contact_phone") or primary.get("contact_phone"),
            "report_time": secondary.get("report_time") or primary.get("report_time"),
            
            # 技术信息优先使用primary
            "solution": primary.get("solution") or secondary.get("solution"),
            "processing_note": primary.get("processing_note") or secondary.get("processing_note"),
            "assignee": primary.get("assignee") or secondary.get("assignee"),
            "actual_hours": primary.get("actual_hours") or secondary.get("actual_hours"),
        }
    ...
    async def _create_maintenance_task(
        ...
        task = RepairTask(
            ...
            status=TaskStatus.COMPLETED if data.get("complete_time") else TaskStatus.PENDING,
    ...
    async def _add_tags_for_maintenance_task(
        ...
        # 检查非默认好评
        if data.get("customer_rating") and data["customer_rating"] not in ["默认好评", "系统默认"]:
    ...
    async def _update_monthly_summaries_for_import(
        ...
        for member_id, year, month in updates_needed:
            try:
                await self.work_hours_service.update_monthly_summary(member_id, year, month)
            except Exception as e:
                logger.warning(f"Failed to update monthly summary for member {member_id}, {year}-{month}: {str(e)}")
    ...
    async def _recalculate_work_hours_for_marked_tasks(
        ...
        for task in tasks:
            ...
            members_to_update.add((task.member_id, task.report_time.year, task.report_time.month))
        ...
        for member_id, year, month in members_to_update:
            try:
                ...
                logger.warning(f"Failed to update monthly summary after rush task marking: {str(e)}")