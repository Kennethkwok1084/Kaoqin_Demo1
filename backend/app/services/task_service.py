...
class TaskService:
    ...
    async def _get_or_create_tag(
        ...
        if not tag:
            ...
            # Set tag_type after instantiation to avoid constructor issues
            if tag_type == "bonus":
                tag.tag_type = TaskTagType.BONUS
            elif tag_type == "penalty":
                tag.tag_type = TaskTagType.PENALTY
            else:
                tag.tag_type = TaskTagType.CATEGORY
            self.db.add(tag)
            await self.db.flush()
