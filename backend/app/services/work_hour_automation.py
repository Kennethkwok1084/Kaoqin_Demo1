...
class WorkHourAutomationService:
    ...
    async def schedule_overdue_detection(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result = {
                ...
                "total_processed": late_response_count + late_completion_count + long_overdue_count
            }
    ...
    async def auto_apply_penalty_tags(self, task_ids: Optional[List[int]] = None) -> Dict[str, Any]:
        """
        自动应用惩罚标签
        ...
        try:
            ...
            if task_ids:
                ...
                # 只处理状态为进行中或已完成的任务
                query = query.where(RepairTask.status.in_([TaskStatus.IN_PROGRESS, TaskStatus.COMPLETED]))
    ...
    async def process_review_bonuses(self) -> Dict[str, Any]:
        ...
        try:
            ...
            query = select(RepairTask).options(selectinload(RepairTask.tags)).where(
                and_(
    ...
    async def recalculate_batch_hours(self, 
        ...
        try:
            ...
            for task in tasks:
                ...
                if old_minutes != new_minutes:
                    ...
                    logger.debug(f"Task {task.id} work minutes: {old_minutes} -> {new_minutes}")
        ...
        try:
            ...
            for task_info in notification_tasks:
                try:
                    ...
                    logger.warning(f"Failed to send notification for task {task_info['task_id']}: {str(e)}")
    ...
    async def _detect_late_response_tasks(self) -> int:
        """检测延迟响应的任务"""
        cutoff_time = datetime.utcnow() - timedelta(hours=settings.RESPONSE_TIMEOUT_HOURS)
    ...
    async def _detect_late_completion_tasks(self) -> int:
        """检测延迟完成的任务"""
        cutoff_time = datetime.utcnow() - timedelta(hours=settings.COMPLETION_TIMEOUT_HOURS)
    ...
    async def _apply_auto_penalties(self, task: RepairTask) -> bool:
        ...
        if (task.status != TaskStatus.PENDING and 
            task.response_time and task.report_time and
            not any(tag.name == "延迟响应" for tag in task.tags)):
        ...
        if (task.status == TaskStatus.COMPLETED and 
            task.completion_time and task.response_time and
            not any(tag.name == "延迟完成" for tag in task.tags)):
    ...
    async def _apply_review_bonuses(self, task: RepairTask) -> bool:
        ...
        # 检查差评惩罚
        if (task.rating <= settings.MAX_RATING_FOR_NEGATIVE and 
            not any(tag.name == "差评" for tag in task.tags)):
            await self.task_service._add_penalty_tag(task, "差评", -60)
            bonuses_applied = True
        
        # 检查非默认好评奖励
        elif (task.rating >= settings.MIN_RATING_FOR_POSITIVE and 
              task.feedback and 
              not self._is_default_feedback(task.feedback) and
              not any(tag.name == "非默认好评" for tag in task.tags)):
    ...
    async def _get_notification_tasks(self) -> List[Dict[str, Any]]:
        ...
        # 获取即将超时的任务（还有2小时就超时）
        warning_time = datetime.utcnow() - timedelta(hours=settings.RESPONSE_TIMEOUT_HOURS - 2)
        
        query = select(RepairTask).options(joinedload(RepairTask.assigned_member)).where(
            and_(
                RepairTask.status == TaskStatus.PENDING,
    ...
    async def _send_task_notification(self, task_info: Dict[str, Any]):
        ...
        if notification_type == "response_warning":
            logger.info(f"Sending response warning notification to {task_info['member_name']} "
        ...
        try:
            ...
            for tag_name in penalty_tags + bonus_tags:
                tag_query = select(func.count()).select_from(
                    select(task_tag_association).join(TaskTag).where(TaskTag.name == tag_name)
                )
