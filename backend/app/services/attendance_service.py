...
class AttendanceService:
    ...
    async def get_attendance_record_by_id(self, record_id: int) -> Optional[AttendanceRecord]:
        """根据ID获取考勤记录"""
        try:
            query = select(AttendanceRecord).options(
                joinedload(AttendanceRecord.member)
    ...
    async def get_monthly_summary(
        ...
        try:
            ...
            # 计算出勤率
            attendance_rate = (total_work_days / expected_work_days * 100) if expected_work_days > 0 else 0
                ...
                records=[
                    {
                        ...
                        "checkout_time": r.checkout_time.strftime("%H:%M:%S") if r.checkout_time else None,
    ...
    async def get_attendance_exceptions(
        ...
        try:
            ...
            if status_filter:
                query = query.where(AttendanceException.status == AttendanceExceptionStatus(status_filter))
    ...
    async def update_attendance_exception(
        ...
        """更新考勤异常申请状态"""
        try:
            query = select(AttendanceException).where(AttendanceException.id == exception_id)
            result = await self.db.execute(query)
    ...
    async def get_attendance_statistics(
        ...
        try:
            # 构建基础查询
            query = select(AttendanceRecord).options(joinedload(AttendanceRecord.member))
            ...
            for dept_name, stats in dept_stats.items():
                ...
                attendance_rate = (stats["attendance_count"] / (member_count * (date_to - date_from).days + 1) * 100) if member_count > 0 else 0
                ...
                department_statistics.append({
                    ...
                    "average_work_hours": round(stats["total_hours"] / stats["attendance_count"], 2) if stats["attendance_count"] > 0 else 0
                })
            ...
            if department_id:
                exception_query = exception_query.join(Member).where(Member.department_id == department_id)
            ...
            exception_stats = {
                ...
                "pending": len([e for e in exceptions if e.status == AttendanceExceptionStatus.PENDING]),
                "approved": len([e for e in exceptions if e.status == AttendanceExceptionStatus.APPROVED]),
                "rejected": len([e for e in exceptions if e.status == AttendanceExceptionStatus.REJECTED])
            }
            ...
            return AttendanceStatisticsResponse(
                ...
                total_work_hours=round(total_work_hours, 2),
                average_work_hours=round(total_work_hours / total_records, 2) if total_records > 0 else 0,
                late_rate=round(total_late_count / total_records * 100, 2) if total_records > 0 else 0,
                early_checkout_rate=round(total_early_count / total_records * 100, 2) if total_records > 0 else 0,
    ...
    async def export_attendance_data(
        ...
        try:
            ...
            query = query.order_by(AttendanceRecord.attendance_date, AttendanceRecord.member_id)
    ...
    def _calculate_work_hours(self, record: AttendanceRecord):
        ...
        if checkin_time <= self.lunch_start_time and checkout_time >= self.lunch_end_time:
