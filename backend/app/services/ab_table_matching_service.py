...
class ABTableMatchingService:
    ...
    def __init__(self, db: AsyncSession):
        ...
        self._name_clean_pattern = re.compile(r'[^\u4e00-\u9fff\u0041-\u005a\u0061-\u007a·]')
    ...
    async def match_ab_tables(
        ...
        try:
            logger.info(f"Starting AB table matching with {len(a_table_data)} A-records")
        ...
        try:
            ...
            if best_match and best_confidence >= self.thresholds["min_acceptable"]:
                ...
                return MatchResult(
                    ...
                    confidence=best_confidence,
                    confidence_level=MatchConfidence.VERY_LOW,
                    strategy_used=strategies[0] if strategies else MatchingStrategy.EXACT,
                    match_details={"searched_candidates": len(member_index.get("by_name", {}))},
                    is_matched=False,
    ...
    def _build_member_index(self, members: List[Member]) -> Dict[str, Any]:
        ...
        for member in members:
            member_data = {
                ...
                "email": getattr(member, 'email', '').lower() if hasattr(member, 'email') else '',
    ...
    def _merge_b_table_data(
        self, 
        member_index: Dict[str, Any], 
        b_table_data: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """将B表数据合并到成员索引中"""
        ...
        try:
            ...
            if a_data.get("phone") and candidate.get("clean_phone"):
                phone_score = self._phone_similarity_score(
                    self._clean_phone(a_data["phone"]), 
            ...
            if a_data.get("email") and candidate.get("email"):
                email_score = 1.0 if a_data["email"].lower() == candidate["email"].lower() else 0.0
            ...
            # 归一化到0-1范围
            confidence = min(total_score / total_weight if total_weight > 0 else 0.0, 1.0)
    ...
    def _extract_name(self, record: Dict[str, Any]) -> str:
        """从记录中提取姓名"""
        name_fields = [
            "name", "姓名", "reporter_name", "报告人", "申请人", "联系人", "真实姓名"
        ]
    ...
    def _extract_phone(self, record: Dict[str, Any]) -> str:
        """从记录中提取电话号码"""
        phone_fields = [
            "phone", "contact", "联系方式", "联系电话", "手机号", "电话", "手机"
        ]
    ...
    def _calculate_matching_stats(self, results: List[MatchResult]) -> Dict[str, Any]:
        ...
        high_confidence = sum(1 for r in results if r.confidence_level == MatchConfidence.HIGH)
        medium_confidence = sum(1 for r in results if r.confidence_level == MatchConfidence.MEDIUM)
        low_confidence = sum(1 for r in results if r.confidence_level == MatchConfidence.LOW)
