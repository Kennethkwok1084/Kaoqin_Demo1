...
async def get_dashboard_overview(
    ...
    try:
        ...
        in_progress_result = await db.execute(
            select(func.count(RepairTask.id)).where(RepairTask.status == TaskStatus.IN_PROGRESS)
        )
        ...
        overview_data = DashboardOverviewResponse(
            ...
            trends={
                ...
                "completedTrend": calculate_trend(completed_this_month, last_month_completed)
            },
            systemInfo={
...
async def get_my_tasks(
    ...
    try:
        ...
        ).order_by(desc(RepairTask.created_at)).limit(limit)
        ...
        total_assigned_result = await db.execute(
            select(func.count(RepairTask.id)).where(RepairTask.assigned_to == current_user.id)
        )
        total_assigned = total_assigned_result.scalar() or 0
        
        pending_count_result = await db.execute(
            select(func.count(RepairTask.id)).where(
                and_(RepairTask.assigned_to == current_user.id, RepairTask.status == TaskStatus.PENDING)
            )
        )
        ...
        in_progress_count_result = await db.execute(
            select(func.count(RepairTask.id)).where(
                and_(RepairTask.assigned_to == current_user.id, RepairTask.status == TaskStatus.IN_PROGRESS)
            )
        )
        ...
        completed_count_result = await db.execute(
            select(func.count(RepairTask.id)).where(
                and_(RepairTask.assigned_to == current_user.id, RepairTask.status == TaskStatus.COMPLETED)
            )
        )
...
@router.get("/recent-activities", response_model=TypedResponse[DashboardActivitiesResponse])
async def get_recent_activities(
    ...
    try:
        # 获取最近创建的任务
        recent_tasks_query = select(RepairTask).options(
        ...
        for task in recent_tasks:
            ...
            if task.created_by_member:
                activities.append({
                    ...
                    "targetType": "task",
                    "createdAt": task.created_at.isoformat() + "Z" if task.created_at else None,
                    "priority": "primary" if task.priority == TaskPriority.HIGH else "info"
                })
                activity_id += 1
        ...
        today_activities = len([a for a in activities if a.get("createdAt", "") >= today_start.isoformat()])
        
        activities_data = DashboardActivitiesResponse(
            activities=activities,
            summary={
                "total": total_activities,
                "todayCount": today_activities
            }
        )
