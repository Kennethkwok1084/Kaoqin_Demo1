    ...
    try:
        ...
        if attendance_stats["active_members"] > 0:
            attendance_stats["average_work_hours"] = round(
                attendance_stats["total_work_hours"] / attendance_stats["active_members"], 2
            )
...
async def get_work_hours_overview(
    ...
    try:
        ...
        if member_id:
            # 个人工时概览
            work_hours_data = await work_hours_service.calculate_monthly_work_hours(member_id, year, month)
            ...
            # 批量更新所有成员的月度汇总
            update_result = await work_hours_service.batch_update_monthly_summaries(year, month)
            ...
            for member in members:
                try:
                    member_data = await work_hours_service.calculate_monthly_work_hours(member.id, year, month)
                    
                    team_summary["total_hours"] += member_data["total_hours"]
                    team_summary["total_repair_hours"] += member_data["repair_task_hours"]
                    team_summary["total_monitoring_hours"] += member_data["monitoring_hours"]
                    team_summary["total_assistance_hours"] += member_data["assistance_hours"]
                    ...
                    logger.warning(f"Failed to get work hours for member {member.id}: {str(e)}")
            
            # 排序（按总工时降序）
            team_summary["member_details"].sort(key=lambda x: x["total_hours"], reverse=True)
...
async def get_work_hours_analysis(
    year: int = Query(..., description="年份"),
    month: int = Query(..., ge=1, le=12, description="月份"),
    analysis_type: str = Query("breakdown", regex="^(breakdown|penalty|bonus|comparison)$", description="分析类型"),
    current_user: Member = Depends(get_current_active_group_leader),
    db: AsyncSession = Depends(get_db)
):
    """
    ...
    try:
        ...
        work_hours_service = WorkHoursCalculationService(db)
        analysis_data = {"analysis_type": analysis_type, "period": {"year": year, "month": month}}
        
        # 获取该月所有成员的工时汇总
        summary_query = select(MonthlyAttendanceSummary).options(
        ...
        if analysis_type == "breakdown":
            ...
            for summary in summaries:
                ...
                breakdown_data["member_breakdown"].append({
                    ...
                    "member_name": summary.member.name if summary.member else "未知",
            ...
            for summary in summaries:
                ...
                penalty_data["late_response_penalty"] += summary.late_response_penalty_hours
                penalty_data["late_completion_penalty"] += summary.late_completion_penalty_hours
                penalty_data["negative_review_penalty"] += summary.negative_review_penalty_hours
            ...
            # 按惩罚时间排序
            penalty_data["penalty_details"].sort(key=lambda x: x["total_penalty"], reverse=True)
            ...
            for summary in summaries:
                ...
                bonus_data["total_positive_review_hours"] += summary.positive_review_hours
            ...
            # 按奖励时间排序
            bonus_data["bonus_details"].sort(key=lambda x: x["total_bonus"], reverse=True)
            ...
            if summaries:
                ...
                comparison_data["averages"] = {
                    ...
                    "avg_monitoring_hours": round(sum_monitoring_hours / total_members, 2),
                    "avg_assistance_hours": round(sum_assistance_hours / total_members, 2)
                }
                ...
                for summary in summaries:
                    comparison_data["member_comparison"].append({
                        ...
                        "member_name": summary.member.name if summary.member else "未知",
                        "total_hours": summary.total_hours,
                        "vs_avg_total": round(summary.total_hours - comparison_data["averages"]["avg_total_hours"], 2),
                        "repair_hours": summary.repair_task_hours,
                        "vs_avg_repair": round(summary.repair_task_hours - comparison_data["averages"]["avg_repair_hours"], 2),
                        "is_full_attendance": summary.total_hours >= 30.0,
                        "efficiency_rank": 0  # 稍后计算排名
                    })
                
                # 计算效率排名
                comparison_data["member_comparison"].sort(key=lambda x: x["total_hours"], reverse=True)
        ...
        logger.info(f"Work hours analysis ({analysis_type}) retrieved by {current_user.student_id}")
        
        return create_response(
            data=analysis_data,
            message=f"工时{analysis_type}分析获取成功"
        )
...
async def batch_update_work_hours_summaries(
    ...
    try:
        ...
        # 执行批量更新
        result = await work_hours_service.batch_update_monthly_summaries(year, month, member_ids)
        
        logger.info(f"Batch work hours summaries updated by {current_user.student_id}: {result}")
...
async def get_work_hours_trend(
    ...
    try:
        ...
        for month_info in trend_months:
            ...
            if member_id:
                # 个人趋势数据点
                personal_summary = next((s for s in month_summaries if s.member_id == member_id), None)
                if personal_summary:
                ...
                full_attendance_count = sum(1 for s in month_summaries if s.total_hours >= 30.0)
                ...
                trend_data["trend_series"].append({
                    ...
                    "avg_hours_per_member": round(team_total / len(month_summaries), 2) if month_summaries else 0
                })
