...
from app.api.deps import get_db, get_current_active_user, get_current_active_admin, create_response
from app.core.security import get_password_hash, verify_password
from app.models.member import Member, UserRole
from app.schemas.member import (
    MemberCreate, MemberUpdate, MemberResponse, MemberListResponse,
    MemberImportRequest, MemberImportResponse, PasswordChangeRequest
)
    ...
    try:
        ...
        if 'username' in update_data and update_data['username'] != member.username:
            query = select(Member).where(
                and_(
                    Member.username == update_data['username'],
                    Member.id != member_id
                )
            )
            result = await db.execute(query)
...
async def delete_member(
    ...
    try:
        ...
        return create_response(
            data={"deleted_id": member_id},
            message=f"成功删除成员：{member_name}"
        )
        ...
        try:
            ...
            if existing_member:
                if import_data.skip_duplicates:
                    ...
                    logger.info(f"跳过重复成员: {member_item.name} ({member_item.student_id or username})")
                    continue
                else:
                    failed_imports += 1
                    duplicate_field = "用户名" if existing_member.username == username else "学号"
                    errors.append(f"第{index+1}行: {duplicate_field}已存在 - {member_item.name}")
                    continue
            ...
            try:
                new_member = Member(
                    ...
                    student_id=member_item.student_id.strip() if member_item.student_id else None,
...
async def change_password(
    ...
    try:
        ...
        return create_response(
            data={"updated_member_id": member_id},
            message="密码修改成功"
        )
...
async def get_member_stats(
    ...
    try:
        ...
        # 按部门统计
        dept_query = select(Member.department, func.count(Member.id)).group_by(Member.department)
...
async def complete_profile(
    ...
    if current_user.id != member_id and current_user.role != UserRole.ADMIN:
        logger.warning(f"User {current_user.id} attempted to complete profile for member {member_id}")
