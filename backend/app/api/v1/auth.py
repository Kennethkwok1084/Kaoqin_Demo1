...
from app.core.security import (
    ...
    verify_token, rate_limiter, validate_password_strength
)
from app.models.member import Member, UserRole
from app.schemas.auth import (
    LoginRequest, LoginResponse, TokenResponse, RefreshTokenRequest,
    ChangePasswordRequest, UserProfileResponse, UserProfileUpdate
)
...
async def login(
    ...
    try:
        ...
        if not user:
            logger.warning(f"Login attempt with non-existent student_id: {login_data.student_id}")
            raise HTTPException(
...
async def update_user_profile(
    ...
    try:
        ...
        for field in sensitive_fields:
            if field in update_data:
                if not current_user.is_admin:
                    logger.warning(f"User {current_user.student_id} attempted to update sensitive field: {field}")
                    raise HTTPException(
                        status_code=status.HTTP_403_FORBIDDEN,
        ...
        return create_response(
            data=current_user.get_safe_dict(),
        ...
        logger.error(f"Profile update error for user {current_user.student_id}: {str(e)}")
        await db.rollback()
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def change_password(
    ...
    try:
        ...
        if not verify_password(password_data.current_password, current_user.password_hash):
            logger.warning(f"Invalid current password for user: {current_user.student_id}")
            raise HTTPException(
        ...
        if not is_strong:
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                detail={"message": "Password does not meet strength requirements", "errors": errors}
            )
        ...
        logger.error(f"Password change error for user {current_user.student_id}: {str(e)}")
        await db.rollback()
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
