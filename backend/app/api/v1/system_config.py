...
async def get_all_system_configs(
    ...
    try:
        ...
        return create_error_response(
            message="获取系统配置失败", details={"error": str(e)}
        )
...
async def get_config_categories(
    ...
    try:
        ...
        return create_error_response(
            message="获取配置分类失败", details={"error": str(e)}
        )
...
async def get_work_hours_config(
    ...
    try:
        ...
        return create_response(data=work_hours_config, message="成功获取工时计算配置")

    except Exception as e:
        logger.error(f"Get work hours config error: {str(e)}")
        return create_error_response(
            message="获取工时配置失败", details={"error": str(e)}
        )
...
async def get_penalties_config(
    ...
    try:
        ...
        return create_response(data=penalties_config, message="成功获取扣时规则配置")

    except Exception as e:
        logger.error(f"Get penalties config error: {str(e)}")
        return create_error_response(
            message="获取扣时配置失败", details={"error": str(e)}
        )
...
async def get_thresholds_config(
    ...
    try:
        ...
        return create_response(data=thresholds_config, message="成功获取时间阈值配置")

    except Exception as e:
        logger.error(f"Get thresholds config error: {str(e)}")
        return create_error_response(
            message="获取时间阈值配置失败", details={"error": str(e)}
        )
...
async def update_system_config(
    ...
    try:
        ...
        if not config_key:
            raise HTTPException(
                status_code=http_status.HTTP_400_BAD_REQUEST, detail="缺少配置键名"
            )
        ...
        if success:
            logger.info(
                f"System config updated by user {current_user.id}: {config_key} = {config_value}"
            )
            return create_response(
        ...
        return create_error_response(
            message="更新系统配置失败", details={"error": str(e)}
        )
...
async def bulk_update_system_configs(
    ...
    try:
        ...
        if result["success"]:
            logger.info(
                f"System configs bulk updated by user {current_user.id}: {result}"
            )
            return create_response(
                data=result, message=f"批量更新完成，成功 {result['success_count']} 项"
            )
        else:
            return create_error_response(
        ...
        return create_error_response(
            message="批量更新系统配置失败", details={"error": str(e)}
        )
...
async def reset_config_to_default(
    ...
    try:
        ...
        if success:
            logger.info(
                f"System config reset to default by user {current_user.id}: {config_key}"
            )
            return create_response(
                data={"config_key": config_key}, message="配置重置为默认值成功"
            )
        else:
            return create_error_response(message="配置重置失败，请检查配置键名是否存在")

    except Exception as e:
        logger.error(f"Reset config to default error: {str(e)}")
        return create_error_response(message="重置配置失败", details={"error": str(e)})
...
async def initialize_system_configs(
    ...
    try:
        ...
        return create_error_response(
            message="初始化系统配置失败", details={"error": str(e)}
        )
...
async def export_system_configs(
    ...
    try:
        ...
        return create_error_response(
            message="导出系统配置失败", details={"error": str(e)}
        )
