...
class RedisCache:
    ...
    def _generate_cache_key(self, key_type: str, **kwargs) -> str:
        """生成缓存键"""
        # 根据参数生成唯一的缓存键
        param_string = "_".join(f"{k}={v}" for k, v in sorted(kwargs.items()) if v is not None)
    ...
    async def delete_pattern(self, pattern: str) -> int:
        ...
        try:
            ...
            logger.warning(f"Cache delete pattern error for pattern {pattern}: {str(e)}")
            return 0
    ...
    async def get_stats_cache(self, cache_type: str, **kwargs) -> Optional[Dict[str, Any]]:
        """获取统计数据缓存"""
        cache_key = self._generate_cache_key(f"{self.stats_prefix}{cache_type}:", **kwargs)
        return await self.get(cache_key)
    
    async def set_stats_cache(self, cache_type: str, data: Dict[str, Any], ttl: Optional[int] = None, **kwargs) -> bool:
        """设置统计数据缓存"""
        cache_key = self._generate_cache_key(f"{self.stats_prefix}{cache_type}:", **kwargs)
    ...
    async def get_member_cache(self, member_id: int, cache_type: str = "profile") -> Optional[Dict[str, Any]]:
        """获取成员数据缓存"""
        cache_key = f"{self.member_prefix}{member_id}:{cache_type}"
        return await self.get(cache_key)
    
    async def set_member_cache(self, member_id: int, data: Dict[str, Any], cache_type: str = "profile", ttl: Optional[int] = None) -> bool:
        """设置成员数据缓存"""
        cache_key = f"{self.member_prefix}{member_id}:{cache_type}"
        ttl = ttl or self.default_ttl
    ...
    async def get_task_cache(self, cache_type: str, **kwargs) -> Optional[Dict[str, Any]]:
        """获取任务数据缓存"""
        cache_key = self._generate_cache_key(f"{self.task_prefix}{cache_type}:", **kwargs)
        return await self.get(cache_key)
    
    async def set_task_cache(self, cache_type: str, data: Dict[str, Any], ttl: Optional[int] = None, **kwargs) -> bool:
        """设置任务数据缓存"""
        cache_key = self._generate_cache_key(f"{self.task_prefix}{cache_type}:", **kwargs)
        ttl = ttl or self.default_ttl
        return await self.set(cache_key, data, ttl)
    ...
    async def cache_or_compute(self, key: str, compute_func, ttl: Optional[int] = None, *args, **kwargs) -> Any:
        """缓存或计算数据"""
        # 先尝试从缓存获取
    ...
    def decorator(func):
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                # 如果格式化失败，使用函数名和参数哈希
                param_hash = hashlib.md5(str(args + tuple(kwargs.items())).encode()).hexdigest()[:8]
        ...
        async def wrapper(*args, **kwargs):
            ...
            for pattern in patterns:
                try:
                    ...
                    logger.warning(f"Failed to format cache invalidation pattern: {pattern}")
