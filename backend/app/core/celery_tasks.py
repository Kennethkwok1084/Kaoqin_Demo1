...
def cleanup_expired_data(self, days_to_keep: int = 90):
    ...
    """
    try:
        logger.info(f"Starting data cleanup task (ID: {self.request.id}, days_to_keep: {days_to_keep})")
...
def generate_daily_statistics(self):
    ...
    """
    try:
        logger.info(f"Starting daily statistics generation task (ID: {self.request.id})")
        ...
        async def _generate_stats():
            async with AsyncSessionLocal() as db:
                ...
                # 生成各种统计报告
                daily_overview = await stats_service.get_overview_statistics(start_time, end_time)
                member_stats = await stats_service.get_member_statistics(date_from=start_time, date_to=end_time)
                task_stats = await stats_service.get_task_statistics(date_from=start_time, date_to=end_time)
...
def weekly_work_hour_recalculation(self):
    ...
    """
    try:
        logger.info(f"Starting weekly work hour recalculation task (ID: {self.request.id})")
...
def export_statistics_report(self, report_type: str, date_from: str, date_to: str, member_ids: Optional[List[int]] = None):
    """
    导出统计报告
    异步生成和导出各种格式的统计报告
    """
    try:
        logger.info(f"Starting statistics report export task (ID: {self.request.id}, type: {report_type})")
        ...
        async def _export_report():
            async with AsyncSessionLocal() as db:
                ...
                if report_type == "overview":
                    data = await stats_service.get_overview_statistics(start_date, end_date)
