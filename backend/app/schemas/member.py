...
class BulkMemberOperation(BaseModel):
    ...
    operation: str = Field(
        ..., 
        regex="^(activate|deactivate|delete|set_role|set_group)$",
        description="操作类型"
    )
    ...
    def validate_operation_data(cls, values):
        ...
        operation_data = values.get('operation_data')
        
        if operation == 'set_role':
            if not operation_data or 'role' not in operation_data:
                raise ValueError('设置角色操作需要提供role参数')
        elif operation == 'set_group':
            if not operation_data or 'group_id' not in operation_data:
