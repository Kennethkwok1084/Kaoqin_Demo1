        ...
        json_schema_extra={
            "example": {
                ...
                "status_history": [
                    {
                        "status": "pending",
                        "changed_at": "2025-01-27T14:00:00",
                        "changed_by": "管理员",
                    },
                    {
                        "status": "in_progress",
                        "changed_at": "2025-01-27T14:30:00",
                        "changed_by": "张三",
                    },
                    {
                        "status": "completed",
                        "changed_at": "2025-01-27T16:30:00",
                        "changed_by": "张三",
                    },
                ],
            }
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                ...
                "has_next": True,
                "has_prev": False,
                "filters_applied": {"status": "pending", "task_type": "repair"},
            }
        }
    )
...
class TaskSearchParams(BaseModel):
    ...
    title: Optional[str] = Field(None, max_length=100, description="标题关键词")
    task_number: Optional[str] = Field(None, max_length=20, description="任务编号")
    ...
    assigned_to: Optional[int] = Field(None, description="执行者筛选")
    created_by: Optional[int] = Field(None, description="创建者筛选")
    tag_ids: Optional[List[int]] = Field(None, description="标签筛选")
    location: Optional[str] = Field(None, max_length=50, description="地点关键词")
    is_rush: Optional[bool] = Field(None, description="紧急任务筛选")
    is_overdue: Optional[bool] = Field(None, description="超期任务筛选")
    date_from: Optional[datetime] = Field(None, description="创建时间起始")
    date_to: Optional[datetime] = Field(None, description="创建时间结束")
    deadline_from: Optional[datetime] = Field(None, description="截止时间起始")
    deadline_to: Optional[datetime] = Field(None, description="截止时间结束")

    @model_validator(mode="after")
    def validate_date_ranges(self) -> "TaskSearchParams":
        """验证日期范围"""
...
class TaskAssignment(BaseModel):
    ...
    assignment_note: Optional[str] = Field(None, max_length=200, description="分配备注")

    model_config = ConfigDict(
        json_schema_extra={
            "example": {"assigned_to": 2, "assignment_note": "请优先处理此紧急任务"}
        }
    )
...
class TaskImportRequest(BaseModel):
    ...
    tasks: List[TaskCreate] = Field(
        ..., min_length=1, max_length=1000, description="任务数据列表"
    )
    auto_assign: bool = Field(default=False, description="是否自动分配执行者")
    default_priority: TaskPriority = Field(
        default=TaskPriority.MEDIUM, description="默认优先级"
    )
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                ...
                "failed_imports": 1,
                "warnings": 1,
                "errors": [{"row": 3, "error": "任务标题不能为空"}],
                "warnings_list": [{"row": 5, "warning": "未指定执行者，已设为待分配"}],
                "imported_tasks": [],
            }
        }
    )
