...
async def test_local_setup():
    ...
    engine = create_async_engine(
        "sqlite+aiosqlite:///test_local.db",
        echo=False  # Reduce verbose output
    )
    ...
    async with async_session() as session:
        ...
        print(f"[OK] User created successfully: {test_user.username} (ID: {test_user.id})")
    
    # Test querying user  
    async with async_session() as session:
        from sqlalchemy import select
        result = await session.execute(select(Member).where(Member.student_id == "STU12345"))
