...
class CoverageQualityGate:
    ...
    def check_minimum_coverage(self) -> Tuple[bool, str]:
        ...
        if current_coverage >= minimum_required:
            return (
                True,
                f"✅ 覆盖率达标: {current_coverage:.2f}% >= {minimum_required}%",
            )
        else:
            return (
                False,
                f"❌ 覆盖率不达标: {current_coverage:.2f}% < {minimum_required}%",
            )
    ...
    def check_coverage_regression(self) -> Tuple[bool, str]:
        ...
        if coverage_change >= regression_threshold:
            ...
            return (
                False,
                f"❌ 覆盖率严重回退: {coverage_change:+.2f}% < {regression_threshold}%",
            )
            ...
            if file_path in self.coverage_data["files"]:
                ...
                if total_lines > 0:
                    ...
                    if coverage_pct >= threshold:
                        ...
                        results.append(
                            f"❌ {file_path}: {coverage_pct:.1f}% < {threshold}%"
                        )
    ...
    def generate_coverage_report(self) -> Dict:
        ...
        return {
            ...
            "missing_lines": self.coverage_data["totals"]["num_statements"]
            - self.coverage_data["totals"]["covered_lines"],
        }
    ...
    def run_quality_gate(self) -> bool:
        ...
        for check_name, check_func in checks:
            ...
            try:
                ...
                results.append(
                    {"name": check_name, "passed": False, "message": error_msg}
                )
                print(error_msg)
                all_passed = False
