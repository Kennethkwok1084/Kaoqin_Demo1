    ...
    for file_path in files_to_check:
        if os.path.exists(file_path):
            ...
            for line in lines:
                ...
                if "# type: ignore" in line and not any(
                    keyword in line for keyword in ["import", "Union", "Optional"]
                ):
                    # Only remove if it's not actually needed
                    clean_line = line.replace("  # type: ignore", "").replace(
                        " # type: ignore", ""
                    )
...
def fix_dict_item_errors():
    ...
    if os.path.exists(file_path):
        ...
        if '"enum_support": enum_support,' in content:
            content = content.replace(
                '"enum_support": enum_support,', '"enum_support": bool(enum_support),'
            )
...
def fix_no_any_return_errors():
    ...
    if os.path.exists(file_path):
        ...
        pattern_replacements = [
            (
                r"def (\w+)\(.*?\) -> dict\[str, Any\]:",
                r"def \1(self, *args, **kwargs) -> Dict[str, Any]:",
            ),
            ...
            if "=" in line and "dict[str, Any]" in line:
                ...
                if "result" in line or "data" in line:
                    ...
                    if "try:" not in line and "except:" not in line:
                        ...
                        ).replace("data = ", "data: Dict[str, Any] = ")
