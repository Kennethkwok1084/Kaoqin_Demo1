...
async def test_system_config():
    ...
    try:
        ...
        try:
            ...
            online_task_minutes = await config_service.get_config_value("work_hours.online_task_minutes")
            offline_task_minutes = await config_service.get_config_value("work_hours.offline_task_minutes")
            rush_bonus_minutes = await config_service.get_config_value("work_hours.rush_bonus_minutes")
            ...
            work_hours_configs = await config_service.get_configs_by_category("work_hours")
            logger.info(f"Work hours category configs count: {len(work_hours_configs)}")
            
            # 测试设置配置值
            logger.info("Testing set config value...")
            success = await config_service.set_config_value("work_hours.online_task_minutes", 45)
            logger.info(f"Set config success: {success}")
            
            # 验证设置结果
            updated_value = await config_service.get_config_value("work_hours.online_task_minutes")
            logger.info(f"Updated online task minutes: {updated_value}")
            
            # 重置为默认值
            logger.info("Testing reset to default...")
            reset_success = await config_service.reset_config_to_default("work_hours.online_task_minutes")
            logger.info(f"Reset to default success: {reset_success}")
            
            # 验证重置结果
            reset_value = await config_service.get_config_value("work_hours.online_task_minutes")
