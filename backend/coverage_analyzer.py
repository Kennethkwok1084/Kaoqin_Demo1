...
def generate_coverage_report():
    ...
    try:
        ...
        cmd = [
            ...
            "tests/core/", "tests/unit/", "tests/services/", "tests/api/",
            "-v", "--tb=short", "--maxfail=10"  # 限制失败数量以加速测试
        ]
        ...
        if files:
            ...
            for file_path, file_data in files.items():
                ...
                if file_name in priority3_modules:
                    priority3_modules[file_name] = file_data['summary']['percent_covered']
                ...
                for module, coverage in priority3_modules.items():
                    if coverage is not None:
                        if coverage >= 90:
                            ...
                            status = "⚠️  需改进"
                            color = "\033[91m"
                        print(f"   {color}{module:<20} {coverage:6.2f}% {status}\033[0m")
