...
def test_ab_table_matching():
    ...
    for task in tasks:
        ...
        for member in members:
            if (
                task["reporter_name"] == member["name"]
                and task["reporter_contact"] == member["contact"]
            ):
                matched += 1
                found_match = True
                break
...
def main():
    ...
    test_cases = [
        ...
        (
            "Mixed scenario",
            MockTask(
                "ONLINE", is_rush=True, has_positive_review=True, late_response=True
            ),
            55,
        ),
    ]
    ...
    match_test_pass = (
        matching_result["match_rate"] >= 60
    )  # Expected at least 60% match rate
