...
async def main():
    ...
    # Summary
    if all(test_results):
        print_success("All tests passed! Permission and rate limiting fixes are working correctly.")
        print_info("Fixed issues:")
        print_info("  - Permission dependency functions now use direct role comparison")
        print_info("  - Rate limiting uses proper decorator pattern with 429 status codes")
        print_info("  - Member model properties work without async context issues")
        print_info("  - Database refresh calls specify attributes to avoid greenlet issues")
        sys.exit(0)
    else:
