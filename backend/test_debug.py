...
async def debug_user_creation():
    ...
    async with AsyncSession(engine, expire_on_commit=False) as new_session:
        ...
        if found_user:
            print(f"Found user by ID: {found_user.id}, {found_user.student_id}, {found_user.name}")
        else:
        ...
        if found_by_student_id:
            print(f"Found user by student_id: {found_by_student_id.id}, {found_by_student_id.student_id}, {found_by_student_id.name}")
        else:
