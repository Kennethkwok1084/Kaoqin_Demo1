...
async def test_remote_integration():
    ...
    try:
        ...
        test_engine = create_async_engine(
            ...
            connect_args={
                "server_settings": {
                    "application_name": "kaoqin_integration_test"
                }
            }
        )
        ...
        async with async_session() as session:
            ...
            result = await session.execute(text("SELECT username, name FROM members WHERE student_id = 'STU12345'"))
        ...
        async with AsyncClient(transport=transport, base_url="http://testserver", timeout=30.0) as client:
            ...
            if login_response.status_code == 200:
                ...
                print(f"[DEBUG] Login response keys: {login_result.keys() if login_result else 'None'}")
                print(f"[DEBUG] Login response data: {login_result.get('data', {}) if login_result else 'None'}")
                
                # 检查不同的token字段位置
                access_token = (login_result.get("access_token") or 
                               login_result.get("data", {}).get("access_token") or
                               login_result.get("data", {}).get("token"))
                ...
                if access_token:
                    ...
                    if profile_response.status_code == 200:
                        ...
                        print(f"[OK] Profile: {profile_data.get('name')} - {profile_data.get('department')}")
                        
                        # 4. 测试其他端点
                        members_response = await client.get("/api/v1/members/", headers=headers)
                        print(f"[TEST] Members list: {members_response.status_code}")
                        
                        # 5. 测试任务端点
                        tasks_response = await client.get("/api/v1/tasks/repair", headers=headers)
