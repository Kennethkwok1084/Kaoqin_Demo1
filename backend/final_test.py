...
def test_basic_algorithms():
    ...
    assert calculate_normal_hours(40, [30], []) == 70, "Normal task bonus calculation failed"
    assert calculate_normal_hours(40, [], [-30]) == 10, "Normal task penalty calculation failed"
...
def test_integration_scenarios():
    ...
    assert result1["final_minutes"] == 15, f"Expected 15, got {result1['final_minutes']}"
    ...
    def process_normal_task(base_minutes, has_good_rating, has_timeout):
        ...
        final_minutes = max(0, base_minutes + sum(bonuses) - sum(abs(p) for p in penalties))
        return {
            "is_rush_order": False,
    ...
    assert result3["final_minutes"] == 70, f"Expected 70, got {result3['final_minutes']}"
    
    # Test normal task with penalty  
    result4 = process_normal_task(40, False, True)
    assert result4["final_minutes"] == 10, f"Expected 10, got {result4['final_minutes']}"
    ...
    def simulate_ab_matching(a_record, b_records):
        ...
        def calculate_similarity(name1, phone1, name2, phone2):
            ...
            name_sim = 1.0 if name1 == name2 else (0.8 if name1 in name2 or name2 in name1 else 0.0)
