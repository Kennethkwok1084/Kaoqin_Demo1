    ...
    try:
        ...
        if "database_url_contains" in expected_patterns:
            ...
            if pattern not in settings.DATABASE_URL:
                print(
                    f"  FAIL: DATABASE_URL should contain '{pattern}', got: {settings.DATABASE_URL}"
                )
                success = False
            else:
        ...
        if "should_use_postgresql" in expected_patterns:
            ...
            if actual != expected:
                print(
                    f"  FAIL: should_use_postgresql should be {expected}, got: {actual}"
                )
                success = False
            else:
...
def main():
    ...
    test_cases = [
        ...
        # Force SQLite tests
        {
            ...
            "env_vars": {"FORCE_SQLITE_TESTS": "true"},
            "expected_patterns": {
                "test_url_contains": "sqlite+aiosqlite",
                "should_use_postgresql": False,
            },
