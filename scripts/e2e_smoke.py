...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('e2e_smoke_test.log')
    ]
)
            ...
            async with self.session.post(
                ...
                if response.status == 200:
                    ...
                    async with self.session.get(
                        f"{self.config.api_base_url}/api/v1/auth/me",
                        headers=headers
                    ) as me_response:
                ...
                if method == "GET":
                    async with self.session.get(url, headers=headers) as response:
                        if response.status in [200, 201]:
                            ...
                            logger.warning(f"⚠️  {description} - 失败 ({response.status})")
                            
            except Exception as e:
                logger.error(f"❌ {description} - 异常: {str(e)}")
        
        success_rate = success_count / len(test_endpoints)
        logger.info(f"核心API测试完成，成功率: {success_rate:.1%} ({success_count}/{len(test_endpoints)})")
    ...
    async def run_all_tests(self) -> Dict[str, bool]:
        ...
        try:
            ...
            async with APISmokeTest(self.config) as api_test:
                ...
                self.results["authentication"] = await api_test.test_authentication_flow()
