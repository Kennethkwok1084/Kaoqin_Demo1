...
def test_env_loading():
    ...
    try:
        ...
        print_test_result("DATABASE_URL_SYNC", db_url_sync is not None, f"DATABASE_URL_SYNC: {db_url_sync}")
...
def test_settings_loading():
    ...
    try:
        ...
        has_db_url_sync = hasattr(settings, 'DATABASE_URL_SYNC') and settings.DATABASE_URL_SYNC
        
        print_test_result("数据库URL配置", has_db_url, f"DATABASE_URL: {getattr(settings, 'DATABASE_URL', 'None')}")
        print_test_result("同步数据库URL配置", has_db_url_sync, f"DATABASE_URL_SYNC: {getattr(settings, 'DATABASE_URL_SYNC', 'None')}")
...
def test_database_engine():
    ...
    try:
        ...
        from app.core.database import async_engine, sync_engine, AsyncSessionLocal, SessionLocal
        
        print_test_result("异步数据库引擎创建", async_engine is not None, "异步数据库引擎对象创建成功")
        print_test_result("同步数据库引擎创建", sync_engine is not None, "同步数据库引擎对象创建成功")
        print_test_result("异步会话工厂创建", AsyncSessionLocal is not None, "异步数据库会话工厂创建成功")
        print_test_result("同步会话工厂创建", SessionLocal is not None, "同步数据库会话工厂创建成功")
...
async def test_members_table():
    ...
    try:
        ...
        print_test_result("Members表存在", table_exists, "members表存在于数据库中" if table_exists else "members表不存在")
        ...
        if table_exists:
            ...
            expected_columns = ['id', 'username', 'name', 'student_id', 'phone', 'department', 'class_name', 'role']
            has_key_columns = all(col in columns for col in expected_columns)
            
            print_test_result("表结构检查", has_key_columns, f"关键字段存在: {has_key_columns}")
            
            # 检查是否有默认admin用户
            async with async_engine.begin() as conn:
                result = await conn.execute(text(
                    "SELECT COUNT(*) FROM members WHERE username = 'admin'"
                ))
                admin_count = result.scalar()
                
            print_test_result("默认admin用户", admin_count > 0, f"admin用户数量: {admin_count}")
